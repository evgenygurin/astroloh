...
def test_kerykeion_service_comprehensive():
    ...
    print(
        f"✅ Natal chart: {len(natal_result.get('planets', []))} planets, {len(natal_result.get('houses', []))} houses"
    )
    ...
    for house_system in house_systems:
        ...
        if "error" in result:
            ...
            print(
                f"✅ {house_system.value}: {len(result.get('houses', []))} houses"
            )
    ...
    if "error" in arabic_result:
        ...
        print(
            f"✅ Arabic parts: {len(arabic_result.get('parts', []))} parts calculated"
        )
    ...
    if "error" in prog_result:
        ...
        print(
            f"✅ Progressions: {len(prog_result.get('progressed_planets', []))} progressed planets"
        )
    ...
    if "error" in solar_result:
        ...
        print(
            f"✅ Solar return: {len(solar_result.get('planets', []))} planets"
        )
    ...
    available_features = sum(
        1 for feature in capabilities.get("features", {}).values() if feature
    )
...
async def test_async_kerykeion_service():
    ...
    print(
        f"✅ Async natal: {len(result.get('planets', []))} planets, {len(result.get('houses', []))} houses"
    )
...
def main():
    ...
    try:
        results.append(
            ("KerykeionService", test_kerykeion_service_comprehensive())
        )
    except Exception as e:
