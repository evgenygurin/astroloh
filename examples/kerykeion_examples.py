...
def example_basic_natal_chart():
    ...
    for planet in key_planets:
        if planet in result["planets"]:
            ...
            print(
                f"  {planet}: {p['sign']} {p['degree_in_sign']:.1f}° (House {p['house']})"
            )
    ...
    print(
        f"🔥 Elements: Fire={elements.get('Fire', 0)}, Earth={elements.get('Earth', 0)}, Air={elements.get('Air', 0)}, Water={elements.get('Water', 0)}"
    )
...
def example_different_house_systems():
    ...
    birth_data = {
        "name": "Лев Толстой",
        "birth_datetime": datetime(
            1828, 9, 9, 6, 0, tzinfo=pytz.UTC
        ),  # September 9, 1828, 6:00 AM
    ...
    for house_system in house_systems:
        result = service.get_full_natal_chart_data(
            **birth_data, house_system=house_system
        )

        if "error" not in result and result["houses"]:
            first_house = result["houses"][1]
            print(
                f"  {house_system.value:15}: {first_house['sign']} {first_house['degree_in_sign']:.1f}°"
            )
...
def example_sidereal_vs_tropical():
    ...
    birth_data = {
        "name": "Махатма Ганди",
        "birth_datetime": datetime(
            1869, 10, 2, 7, 45, tzinfo=pytz.UTC
        ),  # October 2, 1869, 7:45 AM
    ...
    for zodiac_type in zodiac_types:
        result = service.get_full_natal_chart_data(
            **birth_data, zodiac_type=zodiac_type
        )
...
def example_arabic_parts():
    ...
    natal_result = service.get_full_natal_chart_data(
        name="Омар Хайям",
        birth_datetime=datetime(
            1048, 5, 18, 12, 0, tzinfo=pytz.UTC
        ),  # May 18, 1048, noon
    ...
    for part in arabic_result.get("parts", [])[:5]:  # Show first 5 parts
        print(
            f"  {part['name']}: {part['sign']} {part['degree_in_sign']:.1f}° (House {part['house']})"
        )
...
def example_secondary_progressions():
    ...
    for planet_data in result.get("progressed_planets", []):
        print(
            f"  Progressed {planet_data['name']}: {planet_data['sign']} {planet_data['degree_in_sign']:.1f}°"
        )
...
def example_svg_generation():
    ...
    svg_result = service.generate_chart_svg(
        name="Художник",
        birth_datetime=datetime(
            1975, 3, 21, 15, 45, tzinfo=pytz.UTC
        ),  # Spring Equinox
...
async def example_async_operations():
    ...
    try:
        ...
        if "error" not in result:
            print(
                f"✅ Async chart calculated: {len(result.get('planets', []))} planets"
            )
        ...
        print(
            f"✅ Batch processed {successful}/{len(results)} charts successfully"
        )
...
def main():
    ...
    print(
        "💡 These examples demonstrate the full capabilities of the Kerykeion integration."
    )
