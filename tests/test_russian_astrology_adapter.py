...
class TestRussianZodiacSignEnum:
    ...
    def test_grammatical_cases_completeness(self):
        ...
        required_cases = ["ru", "genitive", "dative", "accusative", "instrumental", "prepositional"]
        
        for sign in RussianZodiacSign:
            sign_data = sign.value
            for case in required_cases:
                assert case in sign_data, f"Missing {case} for {sign.name}"
                assert isinstance(sign_data[case], str), f"{case} must be string for {sign.name}"
                assert len(sign_data[case]) > 0, f"Empty {case} for {sign.name}"
...
class TestRussianPlanetEnum:
    ...
    def test_planet_data_structure(self):
        ...
        required_fields = ["ru", "genitive", "dative", "accusative", "instrumental", "prepositional", "keywords", "description"]
        
        for planet in RussianPlanet:
            planet_data = planet.value
            for field in required_fields:
                assert field in planet_data, f"Missing {field} for {planet.name}"
...
class TestRussianAstrologyAdapterInit:
    ...
    def test_stress_mark_dictionary_exists(self, adapter):
        """Test that stress mark dictionary is initialized"""
        stress_dict = adapter.get_stress_marks_dictionary()
        assert isinstance(stress_dict, dict)
        assert len(stress_dict) > 0
        
        # Should contain key astrological terms
        key_terms = ["овен", "телец", "близнецы", "лев", "дева"]
        found_terms = [term for term in key_terms if term in stress_dict]
...
class TestRussianSignDescriptions:
    ...
    def test_get_russian_sign_description_different_cases(self, adapter):
        ...
        for case in cases:
            description = adapter.get_russian_sign_description("Leo", case=case)
            assert isinstance(description, dict)
            assert "name" in description
    ...
    def test_get_russian_sign_description_case_insensitive(self, adapter):
        ...
        assert description1["name"] == description2["name"] == description3["name"]
...
class TestRussianPlanetDescriptions:
    ...
    def test_get_russian_planet_description_different_cases(self, adapter):
        ...
        for case in cases:
            description = adapter.get_russian_planet_description("Sun", case=case)
            assert "name" in description
...
class TestVoiceOptimization:
    ...
    def test_format_for_voice_with_stress_marks(self, adapter):
        ...
        if "мерку́рий" in formatted.lower() or "ретрогра́де" in formatted.lower():
    ...
    def test_astrological_terms_stress_patterns(self, adapter):
        ...
        for term in terms_to_test:
            if term in stress_dict:
                ...
                assert "́" in stressed_term or stressed_term == term  # Has stress mark or unchanged
...
class TestKerykeionIntegration:
    ...
    def test_localize_kerykeion_planet_data(self, adapter):
        ...
        localized = adapter.localize_kerykeion_planet_data(kerykeion_planet_data)
    ...
    def test_localize_kerykeion_aspect_data(self, adapter):
        ...
        kerykeion_aspects = [
            ...
            {"planet1": "Mercury", "planet2": "Venus", "aspect": "Conjunction", "orb": 1.0}
        ]
...
class TestRussianAstrologyAdapterIntegration:
    ...
    def test_full_localization_workflow(self, adapter):
        ...
        astrological_data = {
            "natal_chart": {
                "planets": {
                    "sun": {"name": "Sun", "sign": "Leo", "house": 5, "degree": 22.5},
                    "moon": {"name": "Moon", "sign": "Cancer", "house": 4, "degree": 15.3}
                },
                "aspects": [
        ...
        localized_data = adapter.localize_complete_astrological_data(astrological_data)
    ...
    def test_voice_optimized_interpretation(self, adapter):
        ...
        interpretation = adapter.generate_voice_optimized_interpretation(planet_data)
...
class TestRussianAstrologyAdapterPerformance:
    ...
    def test_localization_performance(self, adapter):
        ...
        large_dataset = {
            "planets": {f"planet_{i}": {"name": "Sun", "sign": "Leo"} for i in range(100)},
            "aspects": [{"planet1": "Sun", "planet2": "Moon", "aspect": "Opposition"} for _ in range(50)]
        }
    ...
    def test_timezone_detection_performance(self, adapter):
        ...
        russian_cities = [
            "Москва", "Санкт-Петербург", "Екатеринбург", "Новосибирск", "Владивосток",
...
class TestRussianAstrologyAdapterSecurity:
    ...
    def test_input_sanitization_city_names(self, adapter):
        ...
        for malicious_input in malicious_inputs:
            ...
            assert result is None or (isinstance(result, dict) and "zone" in result)
    ...
    def test_voice_output_sanitization(self, adapter):
        ...
        potentially_dangerous_text = "Солнце <script>alert('xss')</script> в Льве"
