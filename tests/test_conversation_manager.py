...
class TestConversationManager:
    ...
    async def test_analyze_conversation_patterns(self):
        ...
        mock_conversations = [
            {"intent": "horoscope", "timestamp": datetime.now() - timedelta(days=1)},
            {"intent": "compatibility", "timestamp": datetime.now() - timedelta(days=2)},
            {"intent": "horoscope", "timestamp": datetime.now() - timedelta(days=3)},
        ]
        
        with patch.object(self.conversation_manager, '_get_conversation_history') as mock_history:
    ...
    def test_get_personalized_suggestions(self):
        ...
        assert any("love" in suggestion.lower() or "любов" in suggestion.lower() 
    ...
    def test_get_personalized_suggestions_low_personalization(self):
        ...
        assert any(any(gen in suggestion.lower() for gen in general_suggestions) 
                  for suggestion in suggestions)
