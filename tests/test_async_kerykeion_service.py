...
class TestAsyncKerykeionServiceInit:
    ...
    def test_generate_chart_id_consistency(self, service):
        ...
        id1 = service._generate_chart_id(
            birth_datetime, latitude, longitude, timezone
        )
        id2 = service._generate_chart_id(
            birth_datetime, latitude, longitude, timezone
        )
...
class TestAsyncKerykeionServiceAsync:
    ...
    async def test_calculate_natal_chart_async_without_cache(
    ...
    async def test_calculate_natal_chart_async_with_cache(
        self, service, sample_birth_data
    ):
    ...
    async def test_calculate_with_performance_monitoring(
        self, service, sample_birth_data
    ):
        ...
        with patch.object(
            ...
            assert (
                service.performance_stats["total_operations"]
                > initial_stats["total_operations"]
            )
            assert (
                service.performance_stats["async_operations"]
                > initial_stats["async_operations"]
            )

            # Average calculation time should be set
            assert service.performance_stats["average_calculation_time"] >= 0

    async def test_slow_calculation_detection(
        self, service, sample_birth_data
    ):
    ...
    async def test_error_handling_in_async_calculation(
        self, service, sample_birth_data
    ):
...
class TestAsyncKerykeionServiceWithoutKerykeion:
    ...
    @pytest.mark.asyncio
    async def test_calculate_without_kerykeion_returns_error(
    ...
    def test_is_available_returns_false(self, service_without_kerykeion):
        ...
        assert not service_without_kerykeion.is_available()
...
class TestAsyncKerykeionServiceRealPerformance:
    ...
    async def test_cache_performance_improvement(self, service):
        ...
        start_time = time.time()
        result1 = await service.calculate_natal_chart_async(
            use_cache=True, **birth_data
        )
        time.time() - start_time

        # Second calculation (with cache)
        start_time = time.time()
        result2 = await service.calculate_natal_chart_async(
            use_cache=True, **birth_data
        )
        time.time() - start_time
...
class TestAsyncKerykeionServiceIntegration:
    ...
    async def test_batch_integration_calculation(self, service):
        ...
        assert all(
            "planets" in result for result in results if result is not None
        )
