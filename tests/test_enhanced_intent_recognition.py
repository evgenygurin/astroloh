...
class TestEnhancedIntentRecognition:
    ...
    def test_advanced_date_extraction(self, recognizer, user_context):
        ...
        test_cases = [
            ...
            ("вчера", [(date.today() - datetime.timedelta(days=1)).strftime("%d.%m.%Y")]),
            ("завтра", [(date.today() + datetime.timedelta(days=1)).strftime("%d.%m.%Y")]),
            ("15.03.90", ["15.03.90"]),
    ...
    def test_zodiac_sign_variations(self, recognizer, user_context):
        ...
        test_cases = [
            ...
            ("совместимость льва и девы", [YandexZodiacSign.LEO, YandexZodiacSign.VIRGO]),
            ("телец с водолеем", [YandexZodiacSign.TAURUS, YandexZodiacSign.AQUARIUS])
        ]
