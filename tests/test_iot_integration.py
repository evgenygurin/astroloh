...
class TestIoTDeviceManager:
    ...
    async def test_register_device_success(self, iot_manager, mock_db):
        ...
        # Execute
        result = await iot_manager.register_device(user_id=1, device_data=device_data)
...
class TestSmartLightingService:
    ...
    @pytest.mark.asyncio
    async def test_apply_lunar_lighting_no_devices(self, lighting_service, iot_manager):
    ...
    @pytest.mark.asyncio
    async def test_apply_lunar_lighting_success(self, lighting_service, iot_manager, mock_lunar_service):
        ...
        mock_device.capabilities = {"on_off": True, "brightness": True, "color": True}
...
class TestWearableIntegration:
    ...
    async def wearable_service(self, mock_db, iot_manager, mock_lunar_service):
        ...
        return WearableIntegrationService(mock_db, iot_manager, mock_lunar_service, transit_calculator)

    @pytest.mark.asyncio
    async def test_sync_wearable_data_device_not_found(self, wearable_service, iot_manager):
    ...
    @pytest.mark.asyncio
    async def test_sync_wearable_data_success(self, wearable_service, iot_manager, mock_db):
    ...
    @pytest.mark.asyncio
    async def test_send_wearable_notification_no_devices(self, wearable_service, iot_manager):
        ...
        # Execute
        result = await wearable_service.send_wearable_notification(user_id=1, alert=alert)
...
class TestHomeAutomation:
    ...
    @pytest.fixture
    async def automation_service(self, mock_db, iot_manager, lighting_service, mock_lunar_service):
        ...
        return HomeAutomationService(
            mock_db, iot_manager, lighting_service, mock_lunar_service,
            transit_calculator, horoscope_generator
        )

    @pytest.mark.asyncio
    async def test_create_automation_success(self, automation_service, mock_db):
    ...
    async def test_create_morning_ritual_automation(self, automation_service):
        ...
        automation_service.create_automation = AsyncMock(return_value={
            "success": True,
            "automation_id": 1,
...
class TestIoTProtocols:
    ...
    async def test_matter_commission_device(self):
        ...
        result = await manager.commission_device(
            setup_code="12345678901", discriminator=1234, device_info=device_info
        )
...
class TestIoTAnalytics:
    ...
    @pytest.mark.asyncio
    async def test_analyze_energy_consumption_no_data(self, analytics_service, mock_db):
        ...
        # Execute
        result = await analytics_service.analyze_energy_consumption(user_id=1, period_days=30)
    ...
    async def test_generate_monthly_report_success(self, analytics_service):
        ...
        analytics_service.analyze_energy_consumption = AsyncMock(return_value={
            "success": True, "total_consumption": 100.5, "lunar_correlation": {}
        })
        analytics_service.analyze_wellness_correlations = AsyncMock(return_value={
            "success": True, "overall_wellness_score": 0.8
        })
        analytics_service.generate_automation_insights = AsyncMock(return_value={
            "success": True, "total_automations": 5, "active_automations": 4
        })
...
class TestVoiceIntegration:
    ...
    @pytest.fixture
    async def voice_service(self, mock_db, iot_manager, lighting_service, mock_lunar_service):
        ...
        return SmartHomeVoiceIntegration(
            mock_db, iot_manager, lighting_service, horoscope_generator, mock_lunar_service
        )

    @pytest.mark.asyncio
    async def test_handle_yandex_command_lunar_lighting(self, voice_service, lighting_service):
        """Test Yandex.Station lunar lighting command."""
        # Setup
        lighting_service.apply_lunar_lighting = AsyncMock(return_value={
            "success": True, "phase": "full_moon", "devices_updated": 2
        })
    ...
    @pytest.mark.asyncio
    async def test_handle_google_assistant_intent(self, voice_service, lighting_service):
        """Test Google Assistant intent handling."""
        # Setup
        lighting_service.apply_lunar_lighting = AsyncMock(return_value={
            "success": True, "phase": "new_moon", "devices_updated": 1
        })
    ...
    async def test_get_suggested_commands(self, voice_service, iot_manager):
        ...
        # Execute
        suggestions = await voice_service.get_suggested_commands(user_id=1, context="")
