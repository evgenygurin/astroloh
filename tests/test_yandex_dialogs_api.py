...
from app.models.yandex_models import (
    YandexSession, YandexResponseModel, YandexResponse
)
...
class TestYandexDialogsAPI:
    ...
    def test_yandex_health_endpoint_error(self):
        ...
        with patch('app.api.yandex_dialogs.dialog_handler') as mock_handler:
            mock_handler.session_manager.get_active_sessions_count.side_effect = Exception("Service error")
    ...
    def test_cleanup_sessions_endpoint_error(self):
        ...
        with patch('app.api.yandex_dialogs.dialog_handler') as mock_handler:
            mock_handler.session_manager.cleanup_expired_sessions.side_effect = Exception("Cleanup failed")
    ...
    def test_yandex_webhook_success(self, mock_handler, mock_user_manager_class, mock_get_db):
    ...
    @patch('app.api.yandex_dialogs.UserManager')
    @patch('app.api.yandex_dialogs.dialog_handler')
    def test_yandex_webhook_error_handling(self, mock_handler, mock_user_manager_class, mock_get_db):
        ...
        mock_handler.handle_request = AsyncMock(side_effect=Exception("Processing error"))
