...
class KnownAstronomicalData:
    ...
    FAMOUS_BIRTHS = {
        ...
        "leonardo_da_vinci": {
            "birth_datetime": datetime(1452, 4, 15, 21, 0),  # Historical records vary
            "latitude": 43.7711,  # Vinci, Italy (approximate)
            "longitude": 10.9218,
            "timezone": "Europe/Rome",
            "known_placements": {
                "sun": {"sign": "aries", "approximate_degree": 26},
                "moon": {"sign": "pisces"},  # Estimated from historical analysis
                "mercury": {"sign": "aries"},
                "venus": {"sign": "aries"}
            }
        }
...
class TestZodiacSignAccuracy:
    ...
    def test_zodiac_sign_boundaries_accuracy(self, calculator):
        ...
        for sign_name, boundaries in KnownAstronomicalData.ZODIAC_SIGN_BOUNDARIES.items():
            ...
            try:
                ...
                calculated_sign = calculator.get_zodiac_sign_by_date(middle_date)
                
                # Convert to string for comparison
                calculated_sign_name = calculated_sign.value.lower()
                
                assert sign_name in calculated_sign_name or calculated_sign_name in sign_name, \
                    f"Expected {sign_name}, got {calculated_sign_name} for date {middle_date}"
    ...
    def test_zodiac_cusp_dates_handling(self, calculator):
        ...
        cusp_tests = [
            ...
            (date(2023, 12, 21), ["sagittarius", "capricorn"])  # Winter solstice
        ]
        ...
        for test_date, possible_signs in cusp_tests:
            ...
            sign_match = any(sign in calculated_sign_name or calculated_sign_name in sign 
                           for sign in possible_signs)
            assert sign_match, f"Date {test_date} gave {calculated_sign_name}, expected one of {possible_signs}"
    ...
    def test_leap_year_zodiac_accuracy(self, calculator):
        ...
        # Should be Pisces (Feb 18 - Mar 20 approximately)
        assert "pisces" in calculated_sign_name or calculated_sign_name in "pisces"
...
class TestNatalChartAccuracy:
    ...
    def test_famous_birth_chart_accuracy(self, kerykeion_service):
        ...
        for person_name, birth_data in KnownAstronomicalData.FAMOUS_BIRTHS.items():
            ...
            if "error" in chart_data:
                pytest.skip(f"Chart calculation failed for {person_name}: {chart_data['error']}")
                continue
            ...
            if "sun" in planets and "sun" in known_placements:
                ...
                assert expected_sun_sign in calculated_sun_sign or calculated_sun_sign in expected_sun_sign, \
            ...
            for planet_name in ["moon", "mercury", "venus", "mars"]:
                if planet_name in planets and planet_name in known_placements:
                    calculated_sign = planets[planet_name].get("sign", "").lower()
                    expected_sign = known_placements[planet_name]["sign"].lower()
                    
                    # Allow some variation in historical data
                    if expected_sign in calculated_sign or calculated_sign in expected_sign:
                        continue  # Match found
                    else:
                        # Log the discrepancy but don't fail (historical data may vary)
                        print(f"Note: {person_name} {planet_name} - Expected {expected_sign}, got {calculated_sign}")
    ...
    def test_planetary_degree_accuracy(self, kerykeion_service):
        ...
        if "sun" in planets:
            ...
            expected_degree = einstein_data["known_placements"]["sun"]["approximate_degree"]
            
            # Allow ±5 degrees variation for historical records
            degree_difference = abs(sun_degree - expected_degree)
            assert degree_difference <= 5.0, \
    ...
    def test_house_calculation_accuracy(self, kerykeion_service):
        ...
        chart_data = kerykeion_service.get_full_natal_chart_data(**modern_birth)
        ...
        for i in range(1, 13):
            ...
            assert 0 <= cusp_degree < 360, f"Invalid cusp degree {cusp_degree} for house {i}"
...
class TestMoonPhaseAccuracy:
    ...
    def test_known_new_moon_accuracy(self, calculator):
        ...
        for new_moon_date in KnownAstronomicalData.KNOWN_MOON_PHASES["new_moons"]:
    ...
    def test_known_full_moon_accuracy(self, calculator):
        ...
        for full_moon_date in KnownAstronomicalData.KNOWN_MOON_PHASES["full_moons"]:
...
class TestAspectCalculationAccuracy:
    ...
    def test_major_aspect_detection(self, kerykeion_service):
        ...
        chart_data = kerykeion_service.get_full_natal_chart_data(
            name="Aspect Test",
            birth_datetime=datetime(1990, 8, 15, 12, 0),  # Noon for clear aspects
            latitude=55.7558,
            longitude=37.6176,
            timezone="Europe/Moscow"
        )
        ...
        for aspect in aspects[:5]:  # Test first 5 aspects
            ...
            assert "aspect_type" in aspect or "symbol" in aspect, "Aspect missing type/symbol"
            assert "orb" in aspect, "Aspect missing orb"
            
            # Orb should be reasonable (0-10 degrees typically)
            orb = aspect.get("orb", 0)
            assert 0 <= orb <= 15, f"Aspect orb {orb}° outside reasonable range"
...
class TestCalculationConsistency:
    ...
    def test_zodiac_sign_consistency(self, astrology_calculator, natal_calculator):
        ...
        for test_date in test_dates:
            ...
            basic_sign = astrology_calculator.get_zodiac_sign_by_date(test_date)
            
            # Get sign from natal chart (if available)
            birth_datetime = datetime.combine(test_date, time(12, 0))
            ...
            if "planets" in enhanced_chart and "sun" in enhanced_chart["planets"]:
                enhanced_sun_sign = enhanced_chart["planets"]["sun"].get("sign", "").lower()
                basic_sign_name = basic_sign.value.lower()
                
                # Signs should match (or be close for cusp dates)
                sign_match = (enhanced_sun_sign in basic_sign_name or 
                             basic_sign_name in enhanced_sun_sign)
                
                if not sign_match:
                    # Allow for 1-day variation on cusp dates
                    print(f"Sign inconsistency on {test_date}: basic={basic_sign_name}, enhanced={enhanced_sun_sign}")
    ...
    def test_moon_phase_calculation_stability(self, astrology_calculator):
        ...
        for result in results[1:]:
            assert result["illumination_percent"] == first_result["illumination_percent"], \
                "Moon phase calculations are inconsistent"
            assert result["phase_name"] == first_result["phase_name"], \
    ...
    def test_planetary_position_precision(self, astrology_calculator):
        ...
        for timestamp in precise_timestamps:
            # Get zodiac sign
            zodiac_sign = astrology_calculator.get_zodiac_sign_by_date(timestamp.date())
            
            # Sign should be reasonable for the date
            assert zodiac_sign is not None, f"No zodiac sign calculated for {timestamp}"
            ...
            assert len(moon_phase["phase_name"]) > 0, \
...
class TestLongTermAccuracy:
    ...
    def test_historical_date_accuracy(self, calculator):
        ...
        for historical_date in historical_dates:
            try:
                zodiac_sign = calculator.get_zodiac_sign_by_date(historical_date)
                assert zodiac_sign is not None, f"Failed to calculate sign for {historical_date}"
                ...
                pytest.skip(f"Historical calculation failed for {historical_date}: {e}")
    ...
    def test_future_date_accuracy(self, calculator):
        ...
        for future_date in future_dates:
            try:
                ...
                assert zodiac_sign is not None, f"Failed to calculate sign for {future_date}"
                ...
                pytest.skip(f"Future calculation failed for {future_date}: {e}")