...
from app.api.yandex_dialogs import handle_yandex_request
...
from app.models.yandex_models import (
    YandexRequestModel, YandexRequestData, YandexSession,
    YandexResponseModel, YandexZodiacSign
)
...
class TestAliceWebhookIntegration:
    ...
    def create_alice_request(self, command: str, utterance: str = None) -> Dict[str, Any]:
    ...
    async def test_webhook_greeting_flow(self):
        ...
        for greeting in AliceTestData.VOICE_COMMANDS["greetings"][:2]:
            ...
            with patch('app.api.yandex_dialogs.get_database_session') as mock_db, \
                 patch('app.services.session_manager.SessionManager') as mock_session_mgr:
                
                mock_db.return_value.__aenter__.return_value = AsyncMock()
                mock_session_mgr.return_value.get_user_context = AsyncMock(return_value=None)
                mock_session_mgr.return_value.update_user_context = AsyncMock()
                
                response = await handle_yandex_request(alice_request)
                
                # Should return valid Alice response
                assert "response" in response
                assert "text" in response["response"]
                assert "tts" in response["response"]
                assert response["version"] == "1.0"
                
                # Response should be greeting
                response_text = response["response"]["text"].lower()
                greeting_indicators = ["привет", "добро пожаловать", "здравствуй", "рад"]
                assert any(indicator in response_text for indicator in greeting_indicators)
    ...
    async def test_webhook_horoscope_flow(self):
        ...
        for horoscope_cmd in AliceTestData.VOICE_COMMANDS["horoscopes"][:2]:
            ...
            with patch('app.api.yandex_dialogs.get_database_session') as mock_db, \
                 patch('app.services.session_manager.SessionManager') as mock_session_mgr, \
                 patch('app.services.horoscope_generator.HoroscopeGenerator') as mock_horoscope:
                ...
                mock_horoscope.return_value.generate_horoscope = AsyncMock(return_value={
                ...
                assert any(indicator in response_text.lower() for indicator in russian_indicators)
    ...
    async def test_webhook_compatibility_flow(self):
        ...
        with patch('app.api.yandex_dialogs.get_database_session') as mock_db, \
             patch('app.services.session_manager.SessionManager') as mock_session_mgr, \
             patch('app.services.compatibility_analyzer.CompatibilityAnalyzer') as mock_compat:
            ...
            mock_compat.return_value.analyze_compatibility = AsyncMock(return_value={
                "score": 88,
            ...
            compatibility_indicators = ["совместимость", "отношения", "пара", "%"]
            assert any(indicator in response_text.lower() for indicator in compatibility_indicators)
    ...
    async def test_webhook_error_handling(self):
        ...
        with patch('app.api.yandex_dialogs.get_database_session') as mock_db:
            ...
            assert "response" in response
            assert "text" in response["response"]
            assert response["response"]["end_session"] == False
            
            # Error message should be user-friendly
            error_text = response["response"]["text"].lower()
            harsh_errors = ["exception", "traceback", "error", "failed"]
            assert not any(harsh_error in error_text for harsh_error in harsh_errors)
...
class TestIntentRecognitionIntegration:
    ...
    def test_compatibility_intent_recognition(self, intent_recognizer):
        ...
        for command in compatibility_commands:
            ...
            expected_intents = [YandexIntent.COMPATIBILITY, YandexIntent.SYNASTRY]
    ...
    def test_ai_intent_recognition(self, intent_recognizer):
        ...
        for command in ai_commands:
            ...
            assert result["intent"] in ai_intents or result["intent"] != YandexIntent.UNKNOWN
    
    def test_entity_extraction_accuracy(self, intent_recognizer):
        """Test entity extraction accuracy"""
        test_cases = [
            ("гороскоп для льва на завтра", {"zodiac_sign": "leo", "date": "tomorrow"}),
            ("совместимость овна и рыб", {"zodiac_sign": "aries", "partner_sign": "pisces"}),
            ("что ждет близнецов сегодня", {"zodiac_sign": "gemini", "date": "today"}),
            ("прогноз для девы на неделю", {"zodiac_sign": "virgo", "period": "week"})
        ]
        ...
        for command, expected_entities in test_cases:
            ...
            for entity_type, expected_value in expected_entities.items():
                if entity_type == "zodiac_sign" and entities.get("zodiac_sign"):
                    # Zodiac sign should match
                    extracted_sign = entities["zodiac_sign"]
                    sign_name = extracted_sign.value.lower() if hasattr(extracted_sign, 'value') else str(extracted_sign).lower()
                    assert expected_value in sign_name or sign_name in expected_value
    ...
    def test_voice_preprocessing_accuracy(self, intent_recognizer):
        ...
        voice_error_cases = [
            ("гароскоп для льва", "гороскоп для льва"),  # Speech recognition error
        ...
        for incorrect_input, expected_correction in voice_error_cases:
            ...
            correct_result = intent_recognizer.recognize_intent(expected_correction)
...
class TestYandexGPTIntegration:
    ...
    async def test_yandex_gpt_horoscope_generation(self, yandex_gpt_client):
        ...
        horoscope_data = {
            ...
            "energy_level": 85,
            "planetary_influences": ["jupiter_trine_sun", "venus_sextile_mars"]
        }
        
        with patch.object(yandex_gpt_client, '_make_api_request') as mock_request:
            ...
            result = await yandex_gpt_client.generate_horoscope_response(horoscope_data)
            ...
            # Should be in Russian
            assert any(russian_word in result["text"].lower() 
    ...
    @pytest.mark.asyncio
    async def test_ai_horoscope_service_integration(self, ai_horoscope_service):
        """Test AI horoscope service integration"""
        with patch('app.core.config.settings') as mock_settings, \
             patch.object(ai_horoscope_service, 'yandex_gpt_client') as mock_gpt:
            
            mock_settings.ENABLE_AI_GENERATION = True
            mock_gpt.generate_horoscope_response = AsyncMock(return_value={
    ...
    async def test_ai_consultation_features(self, ai_horoscope_service):
        ...
        for consultation_type in consultation_types:
            with patch.object(ai_horoscope_service, 'yandex_gpt_client') as mock_gpt:
                mock_gpt.generate_consultation_response = AsyncMock(return_value={
                ...
                if "text" in result:
                    ...
                    assert consultation_type in result.get("consultation_type", "")
...
class TestVoiceInterfaceOptimization:
    ...
    def test_tts_text_optimization(self):
        ...
        test_cases = [
            ...
            ("Прогноз: 85% удачи!", "Прогноз: восемьдесят пять процентов удачи"),  # Numbers to words
    ...
    def test_russian_voice_patterns(self):
        ...
        for term in astrological_terms:
            voice_optimized = adapter.format_for_voice(term, add_stress_marks=True)
    ...
    def test_alice_response_format_compliance(self):
        ...
        response = formatter.format_greeting_response("Привет! Добро пожаловать в мир астрологии!")
        ...
        assert response["end_session"] == False  # Keep session active
...
class TestAliceConversationFlows:
    ...
    async def test_multi_turn_horoscope_conversation(self):
        ...
        with patch('app.api.yandex_dialogs.get_database_session') as mock_db, \
            ...
            response_1 = await handle_yandex_request(alice_request_1)
            
            # Should ask for zodiac sign
            response_text = response_1["response"]["text"].lower()
            sign_question_indicators = ["знак", "зодиак", "какой", "скажите"]
            assert any(indicator in response_text for indicator in sign_question_indicators)
        ...
        with patch('app.api.yandex_dialogs.get_database_session') as mock_db, \
            ...
            mock_db.return_value.__aenter__.return_value = AsyncMock()
            mock_session_mgr.return_value.get_user_context = AsyncMock(return_value={
                "awaiting_data": "zodiac_sign"
            })
            mock_session_mgr.return_value.update_user_context = AsyncMock()
            ...
            response_2 = await handle_yandex_request(alice_request_2)
            
            # Should provide horoscope for Leo
            response_text = response_2["response"]["text"].lower()
            leo_indicators = ["лев", "льв", "отличный"]
            assert any(indicator in response_text for indicator in leo_indicators)
    ...
    async def test_conversation_timeout_handling(self):
        ...
        with patch('app.api.yandex_dialogs.get_database_session') as mock_db, \
            ...
            assert not any(error in response_text for error in error_indicators)
    ...
    async def test_context_persistence_across_requests(self):
        ...
        with patch('app.api.yandex_dialogs.get_database_session') as mock_db, \
            ...
            response_1 = await handle_yandex_request(alice_request)
            
            # Context should be updated with zodiac sign
            update_calls = mock_session_mgr.return_value.update_user_context.call_args_list
        ...
        with patch('app.api.yandex_dialogs.get_database_session') as mock_db, \
            ...
            response_2 = await handle_yandex_request(alice_request)
            ...
            assert not any(indicator in response_text for indicator in sign_question_indicators)
