...
class TestWorkflowScenarios:
    ...
    async def test_complete_horoscope_workflow(self):
        ...
        with patch.multiple(
            ...
            ].get_conversation_context.return_value = mock_context
            ...
            mocks["intent_recognition"].recognize_intent.return_value = (
                YandexIntent.GREETING,
                {},
                0.9,
            )
            mocks[
                "dialog_flow_manager"
            ].get_current_state.return_value = DialogState.GREETING
            mocks[
                "dialog_flow_manager"
            ].process_intent.return_value = DialogState.WAITING_FOR_REQUEST
            mocks[
                "response_formatter"
            ].format_greeting_response.return_value = MagicMock()

            greeting_response = await self.dialog_handler.handle_request(
                greeting_request, mock_db
            )
            assert greeting_response is not None

            # Step 2: User requests horoscope
            horoscope_request = self.create_mock_request(
                "мой гороскоп на сегодня"
            )
            mocks["intent_recognition"].recognize_intent.return_value = (
                YandexIntent.PERSONAL_HOROSCOPE,
                {"period": "today"},
                0.9,
            )
            mocks[
                "dialog_flow_manager"
            ].get_current_state.return_value = DialogState.WAITING_FOR_REQUEST
            mocks[
                "dialog_flow_manager"
            ].process_intent.return_value = DialogState.PROVIDING_HOROSCOPE
            ...
            mocks[
                "horoscope_generator"
            ].generate_personal_horoscope.return_value = mock_horoscope
            mocks[
                "response_formatter"
            ].format_horoscope_response.return_value = MagicMock()

            horoscope_response = await self.dialog_handler.handle_request(
                horoscope_request, mock_db
            )
            ...
            assert mocks[
                "horoscope_generator"
            ].generate_personal_horoscope.called
    ...
    async def test_natal_chart_data_collection_workflow(self):
        ...
        with patch.multiple(
            ...
            mocks[
                "dialog_flow_manager"
            ].process_intent.return_value = DialogState.COLLECTING_BIRTH_DATA
            mocks[
                "response_formatter"
            ].format_clarification_response.return_value = MagicMock()

            response1 = await self.dialog_handler.handle_request(
                request1, mock_db
            )
            ...
            mocks[
                "dialog_flow_manager"
            ].process_intent.return_value = DialogState.COLLECTING_BIRTH_TIME

            response2 = await self.dialog_handler.handle_request(
                request2, mock_db
            )
            ...
            mocks[
                ...
                DialogState.COLLECTING_BIRTH_TIME
            )
            mocks[
                "dialog_flow_manager"
            ].process_intent.return_value = (
                DialogState.COLLECTING_BIRTH_LOCATION
            )

            response3 = await self.dialog_handler.handle_request(
                request3, mock_db
            )
            ...
            mocks[
                "dialog_flow_manager"
            ].process_intent.return_value = DialogState.PROVIDING_NATAL_CHART

            mock_natal_chart = {
                "planets": {"sun": {"sign": "taurus", "house": 2}},
                "interpretation": {
                    "personality": "Стабильная и целеустремленная личность"
                },
            }
            mocks[
                "natal_chart"
            ].calculate_natal_chart.return_value = mock_natal_chart
            mocks[
                "response_formatter"
            ].format_natal_chart_response.return_value = MagicMock()

            response4 = await self.dialog_handler.handle_request(
                request4, mock_db
            )
    ...
    async def test_compatibility_analysis_workflow(self):
        ...
        with patch.multiple(
            ...
            mocks[
                "dialog_flow_manager"
            ].process_intent.return_value = DialogState.PROVIDING_COMPATIBILITY
            ...
            ].calculate_compatibility.return_value = mock_compatibility
            mocks[
                "response_formatter"
            ].format_compatibility_response.return_value = MagicMock()
            ...
            mocks[
                ...
                DialogState.PROVIDING_COMPATIBILITY
            )
            mocks[
                "response_formatter"
            ].format_detailed_compatibility_response.return_value = MagicMock()
            ...
            assert mocks[
                "response_formatter"
            ].format_compatibility_response.called
    ...
    async def test_error_recovery_workflow(self):
        ...
        with patch.multiple(
            ...
            mocks[
                "dialog_flow_manager"
            ].process_intent.side_effect = ValueError("Invalid date")
            ...
            mocks[
                "dialog_flow_manager"
            ].process_intent.side_effect = None  # Reset error
            mocks[
                "dialog_flow_manager"
            ].process_intent.return_value = DialogState.PROVIDING_HOROSCOPE
            mocks[
    ...
    async def test_multi_user_session_workflow(self):
        ...
        with patch.multiple(
            ...
            mocks[
                "conversation_manager"
            ].get_conversation_context.side_effect = get_context_side_effect

            # User 1 requests horoscope
            request1 = self.create_mock_request(
                "мой гороскоп", user_id="user1", session_id="session1"
            )
            mocks["intent_recognition"].recognize_intent.return_value = (
                YandexIntent.PERSONAL_HOROSCOPE,
                {"period": "today"},
            ...
            mocks[
                "horoscope_generator"
            ].generate_personal_horoscope.return_value = {
            ...
            request2 = self.create_mock_request(
                "мой гороскоп", user_id="user2", session_id="session2"
            )
            ...
            assert (
                mocks[
                    "conversation_manager"
                ].get_conversation_context.call_count
                == 2
            )
            assert (
                mocks[
                    "horoscope_generator"
                ].generate_personal_horoscope.call_count
                == 2
            )
    ...
    async def test_conversation_state_persistence_workflow(self):
        ...
        with patch.multiple(
            ...
            mocks[
                "dialog_flow_manager"
            ].process_intent.side_effect = track_state_change
            mocks[
                "dialog_flow_manager"
            ].get_current_state.return_value = DialogState.WAITING_FOR_REQUEST
            ...
            for command, expected_intent in interactions:
                ...
                mocks[
                    "response_formatter"
                ].format_thanks_response.return_value = MagicMock()
                mocks[
                    "response_formatter"
                ].format_goodbye_response.return_value = MagicMock()

                response = await self.dialog_handler.handle_request(
                    request, mock_db
                )
            ...
            assert mocks[
                "conversation_manager"
            ].update_conversation_context.call_count == len(interactions)
    ...
    async def test_personalization_improvement_workflow(self):
        ...
        with patch.multiple(
            ...
            def get_context_with_increasing_personalization(user_id, db):
                nonlocal call_count
                mock_context = MagicMock()
                mock_context.personalization_level = personalization_levels[
                    min(call_count, 3)
                ]
            ...
            mocks[
                "conversation_manager"
            ].get_conversation_context.side_effect = (
            ...
            for i in range(4):
                ...
                mocks[
                    ...
                    DialogState.WAITING_FOR_REQUEST
                )
                mocks[
                    "dialog_flow_manager"
                ].process_intent.return_value = DialogState.GREETING
                mocks[
                    "response_formatter"
                ].format_greeting_response.return_value = MagicMock()
    ...
    async def test_data_privacy_workflow(self):
        ...
        with patch.multiple(
            ...
            mocks[
                ...
                DialogState.CONFIRMING_DATA_DELETION
            )
            mocks[
                "response_formatter"
            ].format_data_deletion_confirmation.return_value = MagicMock()
            ...
            mocks[
                "gdpr_compliance"
            ].process_deletion_request.return_value = True
            mocks[
                "response_formatter"
            ].format_data_deletion_success.return_value = MagicMock()

            response2 = await self.dialog_handler.handle_request(
                confirmation_request, mock_db
            )
            assert response2 is not None
