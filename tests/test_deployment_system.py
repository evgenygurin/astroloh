...
from app.services.rollback_system import rollback_system, RollbackStrategy, RollbackTrigger
...
class TestFeatureFlagService:
    ...
    async def test_feature_flag_initialization(self, clean_feature_flags):
        ...
        assert KerykeionFeatureFlags.KERYKEION_NATAL_CHARTS in feature_flags.flags
        
        natal_flag = feature_flags.flags[KerykeionFeatureFlags.KERYKEION_NATAL_CHARTS]
    ...
    async def test_feature_update(self, clean_feature_flags):
        ...
        assert feature_flags.is_feature_enabled(feature_name, "any_user") is False
    ...
    async def test_emergency_rollback(self, clean_feature_flags):
        ...
        assert feature_flags.is_feature_enabled(feature_name, "test_user") is False
...
class TestDeploymentMonitor:
    ...
    async def test_kerykeion_usage_recording(self, clean_deployment_monitor):
        ...
        assert len(deployment_monitor.metrics_history) == 2  # response_time + success_rate
        
        # Find response time metric
        response_metric = next(
            m for m in deployment_monitor.metrics_history 
    ...
    @patch('app.services.deployment_monitor.deployment_monitor.perform_health_checks')
    async def test_health_checks(self, mock_health_checks, clean_deployment_monitor):
...
class TestRollbackSystem:
    ...
    async def test_backup_system_state(self, mock_cache, clean_rollback_system):
    ...
    async def test_manual_rollback(self, mock_feature_flags, clean_rollback_system):
    ...
    async def test_rollback_statistics(self, clean_rollback_system):
        ...
        assert stats["average_recovery_time"] == 15.0  # Only successful rollbacks counted
...
class TestIntegration:
    ...
    async def test_feature_flag_deployment_monitoring_integration(
        ...
        enabled = feature_flags.is_feature_enabled(feature_name, user_id)
    ...
    async def test_rollback_trigger_integration(
        ...
        mock_deployment_monitor.get_deployment_dashboard.return_value = mock_dashboard
        
        # Test rollback need analysis
        rollback_plan = await rollback_system.analyze_rollback_need()
        
        # Should detect need for rollback due to system failure
        assert rollback_plan is not None
        assert rollback_plan.strategy in [RollbackStrategy.IMMEDIATE, RollbackStrategy.FULL_SYSTEM]
