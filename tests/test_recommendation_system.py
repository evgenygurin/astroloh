...
class TestContentBasedFiltering:
    ...
    async def test_get_content_recommendations_empty_user(self, db_session: AsyncSession):
    ...
    async def test_get_content_recommendations_with_user(self, db_session: AsyncSession):
        """Test content recommendations for existing user."""
        content_filter = ContentBasedFiltering(db_session)
        
        # Create test user
        user = User(
            yandex_user_id="test_user",
            zodiac_sign="leo",
            data_consent=True
        )
        db_session.add(user)
...
class TestInterestProfilingSystem:
    ...
    async def test_update_user_profile_with_interactions(self, db_session: AsyncSession):
    ...
    def test_map_content_to_interest(self):
        ...
        interest = profiler._map_content_to_interest(
            "daily", 
            "Расскажите про карьеру и работу"
        )
        assert interest == "career"
        
        interest = profiler._map_content_to_interest(
            "daily",
            "Как дела с здоровьем?"
        )
...
class TestCommunicationStyleAdapter:
    ...
    async def test_adapt_content_style_with_preferences(self, db_session: AsyncSession):
        ...
        assert adapted != content
        assert any(prefix in adapted for prefix in [
            "Звёзды шепчут:", 
    ...
    def test_apply_communication_styles(self):
        ...
        assert any(prefix in friendly for prefix in ["Дорогой друг,", "Милый,", "Дорогой,"])
...
class TestChurnPredictionModel:
    ...
    async def test_predict_churn_risk_inactive_user(self, db_session: AsyncSession):
        ...
        user = User(
            ...
            data_consent=True,
            last_accessed=datetime.utcnow() - timedelta(days=45)  # Inactive for long time
        )
        db_session.add(user)
        await db_session.commit()
        await db_session.refresh(user)
...
class TestABTestManager:
    ...
    async def test_assign_user_to_test_existing_assignment(self, db_session: AsyncSession):
...
class TestRecommendationSystemIntegration:
    ...
    async def test_full_recommendation_workflow(self, db_session: AsyncSession):
        ...
        horoscope = await horoscope_gen.generate_personalized_horoscope(user.id, "daily")
        assert horoscope is not None
...
class TestRecommendationPerformance:
    ...
    async def test_recommendation_generation_performance(self, db_session: AsyncSession):
