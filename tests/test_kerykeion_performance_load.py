...
from app.services.kerykeion_service import KerykeionService, KERYKEION_AVAILABLE
...
class TestKerykeionServicePerformance:
        ...
        return [
            {
                ...
                "birth_datetime": datetime(1990 + i % 30, 1 + i % 12, 1 + i % 28, 12 + i % 12, i % 60),
    ...
    def test_single_chart_calculation_performance(self, service):
        ...
        if calculation_times:
            ...
            assert avg_time < 3.0, f"Average calculation time too slow: {avg_time:.2f}s"
            assert median_time < 2.5, f"Median calculation time too slow: {median_time:.2f}s"
            assert max_time < 5.0, f"Worst calculation time too slow: {max_time:.2f}s"
            
            print(f"Single chart performance: avg={avg_time:.2f}s, median={median_time:.2f}s, max={max_time:.2f}s")
    
    @pytest.mark.skipif(not KERYKEION_AVAILABLE, reason="Kerykeion not available")
    def test_batch_calculation_performance(self, service, sample_birth_data_sets):
        ...
        # Assertions
        assert successful_calculations >= batch_size * 0.8, "Too many failed calculations"
        assert avg_time_per_chart < 4.0, f"Batch calculation too slow: {avg_time_per_chart:.2f}s per chart"
        assert total_time < 30.0, f"Total batch time too slow: {total_time:.2f}s"
        
        print(f"Batch performance: {successful_calculations}/{batch_size} successful, {avg_time_per_chart:.2f}s per chart")
    ...
    def test_memory_usage_stability(self, service):
        ...
        for i in range(20):
            birth_data = {
                ...
                "birth_datetime": datetime(1990, 1 + i % 12, 1 + i % 28, 12, 0),
                "latitude": 55.7558,
                "longitude": 37.6176,
                "timezone": "Europe/Moscow"
            }
        ...
        print(f"Memory stability: {memory_increase:.2f}MB increase after 20 calculations")
...
class TestAsyncKerykeionServicePerformance:
    ...
    async def test_async_calculation_performance(self, service):
        ...
        with patch.object(service.kerykeion_service, 'get_full_natal_chart_data') as mock_calc:
            ...
            for _ in range(5):
                ...
                result = await service.calculate_natal_chart_async(**birth_data)
            ...
            if calculation_times:
                ...
                assert avg_time < 1.0, f"Async overhead too high: {avg_time:.3f}s"
                
                print(f"Async calculation performance: {avg_time:.3f}s average")
    ...
    async def test_concurrent_calculation_performance(self, service):
        ...
        birth_data_sets = [
            {
                ...
                "birth_datetime": datetime(1990 + i, 1 + i % 12, 1 + i % 28, 12, 0),
                "latitude": 55.7558,
                "longitude": 37.6176,
                "timezone": "Europe/Moscow"
            }
        ...
        with patch.object(service.kerykeion_service, 'get_full_natal_chart_data') as mock_calc:
            ...
            assert successful_results >= len(birth_data_sets) * 0.8, "Too many concurrent failures"
            
            print(f"Concurrent performance: {successful_results}/{len(birth_data_sets)} successful in {total_time:.2f}s")
    ...
    async def test_performance_monitoring_integration(self, service):
        ...
        assert final_stats['async_operations'] > initial_stats['async_operations']
        assert final_stats['average_calculation_time'] >= 0
        
        print(f"Performance monitoring: {final_stats['total_operations']} operations tracked")
...
class TestCacheServicePerformance:
    ...
    async def test_cache_hit_performance(self):
        ...
        if hit_times:
            ...
            assert avg_hit_time < 0.01, f"Cache hits too slow: {avg_hit_time:.4f}s average"
            assert max_hit_time < 0.05, f"Worst cache hit too slow: {max_hit_time:.4f}s"
            
            print(f"Cache hit performance: {avg_hit_time:.4f}s average, {max_hit_time:.4f}s max")
    ...
    async def test_cache_miss_performance(self):
        ...
        assert avg_miss_time < 0.01, f"Cache misses too slow: {avg_miss_time:.4f}s average"
        assert max_miss_time < 0.05, f"Worst cache miss too slow: {max_miss_time:.4f}s"
        
        print(f"Cache miss performance: {avg_miss_time:.4f}s average, {max_miss_time:.4f}s max")
    ...
    async def test_cache_storage_performance(self):
        ...
        for i in range(50):
            test_data = {
                "id": i,
                "planets": {f"planet_{j}": {"sign": f"sign_{j}", "degree": j * 10} for j in range(10)},
                "houses": {str(k): {"cusp": k * 30, "sign": f"sign_{k}"} for k in range(1, 13)},
                "aspects": [{"planet1": "sun", "planet2": "moon", "orb": 2.5} for _ in range(20)]
            }
        ...
        assert avg_storage_time < 0.1, f"Cache storage too slow: {avg_storage_time:.4f}s average"
        assert max_storage_time < 0.5, f"Worst cache storage too slow: {max_storage_time:.4f}s"
        
        print(f"Cache storage performance: {avg_storage_time:.4f}s average, {max_storage_time:.4f}s max")
...
class TestSystemLoadTesting:
    ...
    async def test_mixed_workload_performance(self, services):
        ...
        for i in range(5):
            ...
            with patch.object(services["kerykeion"], 'get_full_natal_chart_data') as mock_calc:
                mock_calc.return_value = {"planets": {}, "houses": {}, "aspects": []}
                tasks.append(services["async_kerykeion"].calculate_natal_chart_async(**birth_data))
        ...
        for i in range(3):
            ...
            with patch.object(services["transit"], '_calculate_current_transits') as mock_transit:
                mock_transit.return_value = {"aspects": [], "energy_level": 75}
                tasks.append(services["transit"].get_current_transits_async(
        ...
        assert successful_results >= len(tasks) * 0.8, f"Too many failures: {successful_results}/{len(tasks)}"
        
        print(f"Mixed workload: {successful_results}/{len(tasks)} successful in {total_time:.2f}s")
    ...
    async def test_sustained_load_performance(self, services):
            ...
            while time.time() < end_time:
                try:
                    ...
                    with patch.object(services["kerykeion"], 'get_full_natal_chart_data') as mock_calc:
                        ...
                        result = await services["async_kerykeion"].calculate_natal_chart_async(**birth_data)
        ...
        # Performance assertions
        assert operations_per_second >= 5, f"Sustained throughput too low: {operations_per_second:.2f} ops/sec"
        assert error_rate < 0.1, f"Error rate too high: {error_rate:.2%}"
        
        print(f"Sustained load: {operations_per_second:.2f} ops/sec, {error_rate:.2%} error rate")
    ...
    def test_memory_usage_under_load(self, services):
            ...
            for i in range(10):  # 10 operations per batch
                try:
                    birth_data = {
                        ...
                        "birth_datetime": datetime(1990 + batch, 1 + i % 12, 1 + i % 28, 12, 0),
                        "latitude": 55.7558,
                        "longitude": 37.6176,
                        "timezone": "Europe/Moscow"
                    }
                    ...
                    chart_data = services["kerykeion"].get_full_natal_chart_data(**birth_data)
        ...
        print(f"Memory under load: {memory_growth:.2f}MB growth, {memory_peak:.2f}MB peak")
...
class TestPerformanceRegressionDetection:
    ...
    def test_baseline_performance_metrics(self):
        ...
        if times:
            ...
            # Basic sanity checks
            assert baseline_metrics["average_time"] < 10.0, "Baseline performance too slow"
            assert baseline_metrics["p95_time"] < 15.0, "95th percentile too slow"
    ...
    def test_performance_regression_detection(self):
        ...
        if current_times:
            ...
            current_p95 = sorted(current_times)[int(len(current_times) * 0.95)] if len(current_times) > 1 else current_times[0]
            
            # Check for regression (50% slower than baseline)
            regression_threshold = 1.5
            
            time_regression = current_average / baseline_metrics["average_time"]
            p95_regression = current_p95 / baseline_metrics["p95_time"]
            
            # Regression detection
            assert time_regression < regression_threshold, \
            ...
            print(f"Regression check: {time_regression:.2f}x avg time, {p95_regression:.2f}x p95 time")
...
class TestStressAndReliabilityTesting:
    ...
    def test_extended_runtime_stability(self):
        ...
        if operation_times:
            ...
            time_std_dev = statistics.stdev(operation_times) if len(operation_times) > 1 else 0
        else:
        ...
        assert avg_operation_time < 5.0, f"Average operation time degraded: {avg_operation_time:.2f}s"
        assert time_std_dev < 2.0, f"Operation time too variable: {time_std_dev:.2f}s std dev"
        
        print(f"Extended stability: {success_rate:.2%} success rate, "
        ...
        try:
            ...
            birth_data_sets = [
                {
                    ...
                    "birth_datetime": datetime(1990 + i % 30, 1 + i % 12, 1 + i % 28, 12, 0),
                    "latitude": 55.7558,
                    "longitude": 37.6176,
                    "timezone": "Europe/Moscow"
                }
            ...
            async def resource_test():
                ...
                for birth_data in birth_data_sets:
                    with patch.object(service.kerykeion_service, 'get_full_natal_chart_data') as mock_calc:
                        mock_calc.return_value = {"planets": {}, "houses": {}, "aspects": []}
                        tasks.append(service.calculate_natal_chart_async(**birth_data))
            ...
            assert successful_results >= large_batch_size * 0.7, \
                f"Poor recovery from resource pressure: {successful_results}/{large_batch_size} successful"
            
            print(f"Resource recovery: {successful_results}/{large_batch_size} operations completed under pressure")
