...
try:
    ...
    logger.warning(
        f"ENHANCED_TRANSIT_SERVICE_INIT: Kerykeion transits not available - {e}"
    )
...
class TransitService:
    ...
    async def get_current_transits(
        ...
        op_id = performance_monitor.start_operation(
            "TransitService", "get_current_transits"
        )

        try:
            logger.info(
                f"ENHANCED_TRANSIT_SERVICE_CURRENT_START: {transit_date.strftime('%Y-%m-%d')}"
            )
            ...
            if self.is_available():
                result = await self._get_kerykeion_transits_async(
                    natal_chart, transit_date, include_minor_aspects
                )
            else:
                logger.warning(
                    "ENHANCED_TRANSIT_SERVICE_FALLBACK: Using basic calculator"
                )
                result = await self._get_basic_transits_async(
                    natal_chart, transit_date
                )
            ...
            performance_monitor.end_operation(
                op_id, success=True, cache_hit=False
            )
            return result
    ...
    def _get_kerykeion_transits(
        ...
        try:
            ...
            birth_datetime = datetime.fromisoformat(
                natal_chart.get("birth_datetime", "2000-01-01T12:00:00")
            )
            ...
            if not natal_subject:
                logger.error(
                    "ENHANCED_TRANSIT_SERVICE_NATAL_ERROR: Failed to create natal subject"
                )
            ...
            if EphemerisDataFactory:
                ...
                if TransitsTimeRangeFactory:
                    ...
                    processed_transits = (
                        self._process_kerykeion_transit_results(
                            transit_results,
                            transit_date,
    ...
    def _process_kerykeion_transit_results(
        ...
        for result in transit_results:
            ...
            if not include_minor_aspects and aspect_angle not in [
                0,
                60,
                90,
                120,
    ...
    def _extract_transit_info(
        self, transit_result: Any, target_date: datetime
        ...
        try:
            ...
            return {
                "transit_planet": getattr(
                    transit_result, "transiting_planet", "Unknown"
                ),
                "natal_planet": getattr(
                    transit_result, "natal_planet", "Unknown"
                ),
                ...
                "peak_date": getattr(
                    transit_result, "exact_date", target_date
                ).isoformat(),
    ...
    def _get_basic_transits(
        self, natal_chart: Dict[str, Any], transit_date: datetime
    ) -> Dict[str, Any]:
        ...
        current_positions = self.astro_calculator.calculate_planet_positions(
            transit_date
        )
                ...
                aspects.append(
                    {
                        ...
                        "influence": self._get_enhanced_transit_influence(
                            transit_planet,
                            natal_planet,
                            self._get_aspect_name(aspect_angle),
                        ),
                        "strength": self._calculate_enhanced_aspect_strength(
    ...
    async def get_period_forecast(
        ...
        # Start performance monitoring
        op_id = performance_monitor.start_operation(
            "TransitService", "get_period_forecast"
        )

        try:
            logger.info(
                f"ENHANCED_TRANSIT_SERVICE_PERIOD_START: {days} days from {start_date}"
            )
            ...
            for i in range(days):
                ...
                task = self.get_current_transits(
                    natal_chart, forecast_date, use_cache=use_cache
                )
            ...
            for i in range(0, len(forecast_tasks), batch_size):
                ...
                for forecast_date, daily_transits in batch_results:
                    daily_forecast = {
                        ...
                        "energy_level": self._calculate_daily_energy(
                            daily_transits
                        ),
                        "main_influences": daily_transits.get(
                            "daily_influences", []
                        )[:3],
                            ...
                            important_dates.append(
                                {
                                    ...
                                    "significance": transit.get(
                                        "influence",
                                        "Важное транзитное влияние",
                                    ),
                                }
                            )

                    # Собираем общие темы
                    for influence in daily_transits.get(
                        "daily_influences", []
                    ):
    ...
    async def get_important_transits(
        ...
        # Start performance monitoring
        op_id = performance_monitor.start_operation(
            "TransitService", "get_important_transits"
        )

        try:
            logger.info(
                f"ENHANCED_TRANSIT_SERVICE_IMPORTANT_RANGE: {start_date} to {end_date}"
            )

            # Generate cache key
            natal_chart_id = self._generate_chart_cache_key(natal_chart)
            cache_key = (
                f"{natal_chart_id}_important_{lookback_days}_{lookahead_days}"
            )
            ...
            while current_date <= end_date:
                task = self.get_current_transits(
                    natal_chart, current_date, use_cache=use_cache
                )
                            ...
                            major_transits.append(
                                {
                                    **transit,
                                    ...
                                    "planet_speed": self._get_planet_speed(
                                        transit_planet
                                    ),
            ...
            result = {
                ...
                "transformation_timeline": self._create_transformation_timeline(
                    important_transits
                ),
                "preparation_advice": self._get_preparation_advice(
                    important_transits
                ),
                "spiritual_guidance": self._get_spiritual_guidance(
                    important_transits
                ),
            }
            ...
            if use_cache and self.enable_caching:
                await astro_cache.set(
                    cache_key,
                    result,
                    self.cache_ttl_hours["important_transits"] * 3600,
                )
    ...
    def _get_enhanced_transit_influence(
        ...
        detailed_influences = {
            (
                ...
                "Sun",
                "Trine",
            ): "Период расширения личной силы и уверенности в себе. Удачное время для лидерства и новых начинаний.",
            (
                "Saturn",
                "Moon",
                "Square",
            ): "Эмоциональные ограничения и необходимость структурировать чувственную сферу. Время дисциплины в отношениях.",
            (
                "Uranus",
                "Mercury",
                "Conjunction",
            ): "Революционные идеи и неожиданные озарения. Возможны резкие изменения в мышлении и коммуникации.",
            (
                "Neptune",
                "Venus",
                "Opposition",
            ): "Иллюзии в любви и творчестве. Важно различать фантазии и реальность в отношениях.",
            (
                "Pluto",
                "Mars",
                "Square",
            ): "Интенсивная трансформация энергии и действий. Возможны силовые конфликты и внутренние революции.",
        }

        key = (transit_planet, natal_planet, aspect)
        return detailed_influences.get(
            key,
            f"Взаимодействие {transit_planet} и {natal_planet} через {aspect}",
        )

    def _calculate_enhanced_aspect_strength(
    ...
    def _create_enhanced_transit_summary(
        ...
        for transit in active_transits:
            ...
            if planet:
                planet_influences[planet] = (
                    planet_influences.get(planet, 0) + 1
                )

        dominant_planet = (
            max(planet_influences, key=planet_influences.get)
        ...
        harmonious_count = sum(
            ...
            if t.get("nature", "")
            in ["гармоничная возможность", "естественный поток"]
        )
        challenging_count = sum(
            1
            for t in active_transits
            if t.get("nature", "")
            in ["динамическое напряжение", "поляризация и осознание"]
        )
        ...
        if dominant_planet:
            ...
            if harmonious_count > challenging_count:
                ...
                return (
                    f"Интенсивный период {theme} с важными вызовами для роста"
                )
            else:
    ...
    def _get_enhanced_daily_influences(
        self, active_transits: List[Dict[str, Any]]
    ) -> List[str]:
        ...
        strong_transits = [
            t
            for t in active_transits
            if t.get("strength", "") in ["сильный", "очень сильный"]
        ][:3]
    ...
    def _get_timing_recommendations(
        ...
        for transit in active_transits[:5]:  # Топ-5 транзитов
            ...
            timing_advice = self._get_specific_timing_advice(
    ...
    def _get_daily_recommendations(
        self, daily_transits: Dict[str, Any]
    ) -> List[str]:
        """Получает рекомендации на день."""
        timing_recs = daily_transits.get("timing_recommendations", [])
        energy_recs = daily_transits.get("energy_assessment", {}).get(
            "recommendations", []
        )
    ...
    def _create_period_summary(
        ...
        energy_levels = [
            f.get("energy_level", "умеренная") for f in daily_forecasts
        ]
        high_energy_days = sum(
            1 for e in energy_levels if e in ["высокая", "повышенная"]
        )
        ...
        if high_energy_ratio >= 0.7:
            ...
            return "Сбалансированный период с чередованием активности и спокойствия"
        else:
            return "Спокойный период, подходящий для внутренней работы и планирования"
    ...
    def _get_period_advice(self, themes: set) -> str:
        ...
        return theme_advice.get(
            primary_theme,
            "Оставайтесь открытыми к новым возможностям и следуйте интуиции",
        )
    ...
    def _get_planet_speed(self, planet: str) -> str:
        ...
        speeds = {
            ...
            "Jupiter": "медленная",
            "Saturn": "медленная",
            "Uranus": "очень медленная",
            "Neptune": "очень медленная",
            "Pluto": "очень медленная",
        }
    ...
    def _assess_transformation_level(
        self, transit_planet: str, aspect: str
    ) -> int:
    ...
    def _extract_life_themes(
        self, important_transits: List[Dict[str, Any]]
    ) -> List[str]:
    ...
    def _create_transformation_timeline(
        self, important_transits: List[Dict[str, Any]]
    ) -> List[Dict[str, Any]]:
    ...
    def _get_preparation_advice(
        ...
        for transit in important_transits[:3]:  # Топ-3 важных
            ...
            preparation_tips = {
                (
                    "Pluto",
                    "Квадрат",
                ): "Готовьтесь к глубоким внутренним изменениям. Отпустите устаревшее.",
                (
                    "Saturn",
                    "Оппозиция",
                ): "Время подвести итоги и принять ответственность за свой путь.",
                (
                    "Uranus",
                    "Соединение",
                ): "Будьте открыты неожиданным возможностям и изменениям.",
            }
        ...
        if not advice:
            advice.append(
                "Сохраняйте гибкость и доверяйте процессу жизненных изменений"
            )

        return advice

    def _get_spiritual_guidance(
        self, important_transits: List[Dict[str, Any]]
    ) -> str:
        """Получает духовное руководство по важным транзитам."""
        if not important_transits:
            return (
                "Время для внутренней тишины и прислушивания к своей мудрости"
            )
    ...
    def _get_energy_recommendations(self, level: float) -> List[str]:
        ...
        if level >= 80:
            ...
            return [
                "Хорошее время для активных действий",
                "Планируйте важные встречи",
            ]
        elif level >= 40:
            return [
                "Сохраняйте баланс активности и отдыха",
            ...
            return [
                ...
                "Восстановите силы перед активностью",
            ]

    def _get_specific_timing_advice(
        self, planet: str, aspect: str, natal_planet: str
    ) -> Optional[str]:
        ...
        timing_combinations = {
            (
                ...
                "Трин",
                "Sun",
            ): "Отличное время для начинания новых проектов",
            (
                "Mars",
                "Квадрат",
                "Moon",
            ): "Избегайте эмоциональных решений, подождите 2-3 дня",
            (
                "Venus",
                "Секстиль",
                "Venus",
            ): "Благоприятное время для романтических отношений",
            (
                "Saturn",
                "Оппозиция",
                "Mercury",
            ): "Отложите важные решения, проанализируйте детально",
            (
                "Mercury",
                "Соединение",
                "Sun",
            ): "Хорошее время для переговоров и общения",
        }
    ...
    async def _get_kerykeion_transits_async(
        ...
        try:
            ...
            if self.async_kerykeion.is_available():
                # Создаем натальную карту как AstrologicalSubject
                birth_datetime = datetime.fromisoformat(
                    natal_chart.get("subject_info", {}).get(
                        "birth_datetime", "2000-01-01T12:00:00"
                    )
                )
                coordinates = natal_chart.get("subject_info", {}).get(
                    "coordinates", {"latitude": 55.7558, "longitude": 37.6176}
                )

                natal_subject_data = (
                    await self.async_kerykeion.get_full_natal_chart_data(
                        name="TransitChart",
                        birth_datetime=birth_datetime,
                        latitude=coordinates["latitude"],
                        longitude=coordinates["longitude"],
                ...
                if natal_subject_data.get("error"):
                    logger.error(
                        f"ENHANCED_TRANSIT_SERVICE_ASYNC_NATAL_ERROR: {natal_subject_data['error']}"
                    )
                    return await self._get_basic_transits_async(
                        natal_chart, transit_date
                    )

                # Формируем результат с использованием асинхронных методов
                processed_transits = await self._process_async_transit_results(
                    natal_subject_data, transit_date, include_minor_aspects
                )
            ...
            logger.error(
                f"ENHANCED_TRANSIT_SERVICE_ASYNC_KERYKEION_ERROR: {e}"
            )
            return await self._get_basic_transits_async(
                natal_chart, transit_date
            )
    ...
    async def get_performance_stats(self) -> Dict[str, Any]:
        ...
        service_stats = performance_monitor.get_service_statistics(
            "TransitService"
        )
        ...
        if cache_type == "all":
            ...
            for key_pattern in transit_keys:
                try:
                    ...
                    logger.warning(
                        f"ENHANCED_TRANSIT_SERVICE_CLEAR_ERROR: {key_pattern} - {e}"
                    )
