...
class AIContentFilter:
    ...
    def __init__(self):
        ...
        logger.info(
            "AI_CONTENT_FILTER_INITIALIZED: Filter ready for content validation"
        )

    def validate_content(
        self, content: str, consultation_type: str = "general"
    ) -> Dict[str, Any]:
        """
        ...
        """
        logger.info(
            f"AI_CONTENT_FILTER_VALIDATE_START: type={consultation_type}, length={len(content)}"
        )
            ...
            for pattern in self.financial_risk_patterns:
                if re.search(pattern, content, re.IGNORECASE | re.UNICODE):
                    safety_issues.append(
                        f"Risky financial advice detected: {pattern}"
                    )
        ...
        filtered_content = (
            self._filter_content(content, safety_issues)
        ...
        result = {
            ...
            "recommendation": self._get_content_recommendation(
    ...
    def _assess_quality(self, content: str, consultation_type: str) -> float:
        ...
        if re.search(
            r"рекомендуе|советуе|предлага|стоит|лучше|важно",
            content,
            re.IGNORECASE,
        ):
            score += 10
    ...
    def _needs_disclaimer(self, content: str, consultation_type: str) -> bool:
        ...
        if re.search(
            r"обязательно|точно|гарантирую|100%|наверняка",
            content,
            re.IGNORECASE,
        ):
            return True
    ...
    def _get_content_recommendation(
        ...
        if safety_level == ContentSafetyLevel.WARNING:
            return (
                "USE_WITH_CAUTION: Content has minor issues, consider review"
            )
    ...
    def add_appropriate_disclaimer(
        self, content: str, consultation_type: str
    ) -> str:
        ...
        if any(
            ...
            for phrase in [d.lower() for d in self.disclaimer_phrases]
        ):
        ...
        if len(content + disclaimer) <= 1020:  # Leave room for Alice limits
            ...
            truncated_content = (
                content[:max_content_length].rsplit(" ", 1)[0] + "..."
            )
