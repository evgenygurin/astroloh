...
class ServiceStats:
    ...
    recent_operations: Deque[PerformanceMetric] = field(default_factory=lambda: deque(maxlen=100))
...
class PerformanceMonitor:
    ...
    def __init__(self, max_metrics_history: int = 1000):
        ...
        logger.info("PERFORMANCE_MONITOR_INIT: Performance monitoring initialized")

    async def start_monitoring(self):
        """Start background monitoring tasks."""
        if not self._monitor_task:
            self._monitor_task = asyncio.create_task(self._system_monitor_loop())
            logger.info("PERFORMANCE_MONITOR_START: Background monitoring started")
    ...
    async def stop_monitoring(self):
        ...
        if self._monitor_task:
            ...
            try:
                ...
                pass
            logger.info("PERFORMANCE_MONITOR_STOP: Background monitoring stopped")
    ...
    def start_operation(self, service_name: str, operation_name: str) -> str:
        ...
        operation_id = f"{service_name}_{operation_name}_{int(time.time() * 1000000)}"
    ...
    def end_operation(
        ...
        if not hasattr(self, '_active_operations'):
            logger.warning(f"PERFORMANCE_MONITOR_NO_ACTIVE_OPS: {operation_id}")
            return
        ...
        if success:
            ...
            if error_message:
                stats.error_counts[error_message] = stats.error_counts.get(error_message, 0) + 1
        ...
        logger.debug(f"PERFORMANCE_MONITOR_END_OP: {operation_id} in {duration:.3f}s")

    def _check_performance_alerts(self, service_name: str, metric: PerformanceMetric):
    ...
    def get_service_statistics(self, service_name: str) -> Dict[str, Any]:
        ...
        # Top errors
        top_errors = sorted(stats.error_counts.items(), key=lambda x: x[1], reverse=True)[:5]
    ...
    def get_overall_statistics(self) -> Dict[str, Any]:
        ...
        overall_cache_rate = (total_cache_hits / total_operations * 100) if total_operations > 0 else 0
    ...
    async def _system_monitor_loop(self):
        ...
        while self.monitoring_active:
            try:
                ...
                if system_metric["cpu_percent"] > self.alert_thresholds["high_cpu_percent"]:
                    logger.warning(f"PERFORMANCE_ALERT_SYSTEM_CPU: {system_metric['cpu_percent']:.1f}%")
                
                if system_metric["memory_mb"] > self.alert_thresholds["high_memory_mb"]:
                    logger.warning(f"PERFORMANCE_ALERT_SYSTEM_MEMORY: {system_metric['memory_mb']:.1f}MB")
    ...
    def reset_statistics(self, service_name: Optional[str] = None):
        ...
        if service_name:
            if service_name in self.service_stats:
                ...
                logger.info(f"PERFORMANCE_MONITOR_RESET: Statistics reset for {service_name}")
        else:
