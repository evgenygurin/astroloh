...
try:
    from kerykeion import AstrologicalSubject, NatalChart, KerykeionChartSVG
    from kerykeion.utilities import get_houses_list, get_planets_list
    from kerykeion.astrological_subject import KerykeionSubject
    KERYKEION_AVAILABLE = True
    logger.info("KERYKEION_SERVICE_INIT: Kerykeion fully available")
except ImportError as e:
    logger.warning(f"KERYKEION_SERVICE_INIT: Kerykeion not available - {e}")
...
class KerykeionService:
    ...
    def __init__(self):
        ...
        if not self.available:
            logger.warning("KERYKEION_SERVICE: Service initialized without Kerykeion")
    ...
    def create_astrological_subject(
        ...
        if not self.available:
            logger.error("KERYKEION_SERVICE_CREATE_SUBJECT: Kerykeion not available")
            return None
        
        try:
            logger.info(f"KERYKEION_SERVICE_CREATE_SUBJECT: Creating subject for {name}")
            ...
            try:
                ...
                logger.warning(f"KERYKEION_SERVICE_CREATE_SUBJECT: Timezone error {e}, using UTC")
                if birth_datetime.tzinfo is None:
    ...
    def get_full_natal_chart_data(
        ...
        try:
            ...
            for planet in ["sun", "moon", "mercury", "venus", "mars", "jupiter", 
                          ...
                          "true_node", "mean_apog", "osculating_apog", "chiron"]:
                if hasattr(subject, planet):
                    planet_info = getattr(subject, planet)
                    if planet_info:
                        planets_data[planet] = {
                            "name": planet_info.get("name", planet.capitalize()),
                            ...
                            "degree_in_sign": planet_info.get("deg_in_sign", 0),
            ...
            if hasattr(subject, "first_house") and subject.first_house:
                angles["ascendant"] = {
                    ...
                    "degree_in_sign": subject.first_house.get("deg_in_sign", 0),
                }
            ...
            if hasattr(subject, "tenth_house") and subject.tenth_house:
                angles["midheaven"] = {
                    ...
                    "degree_in_sign": subject.tenth_house.get("deg_in_sign", 0),
                }
            ...
            result = {
                ...
                "quality_distribution": self._calculate_quality_distribution(planets_data),
                "dominant_planets": self._find_dominant_planets(planets_data, aspects_data),
            }
    ...
    def calculate_kerykeion_aspects(self, subject: Any) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            aspect_definitions = {
                0: {"name": "Conjunction", "symbol": "☌", "orb": 10, "color": AspectColor.CONJUNCTION.value},
                60: {"name": "Sextile", "symbol": "⚹", "orb": 6, "color": AspectColor.SEXTILE.value},
                90: {"name": "Square", "symbol": "□", "orb": 8, "color": AspectColor.SQUARE.value},
                120: {"name": "Trine", "symbol": "△", "orb": 9, "color": AspectColor.TRINE.value},
                180: {"name": "Opposition", "symbol": "☍", "orb": 10, "color": AspectColor.OPPOSITION.value},
                30: {"name": "Semisextile", "symbol": "⚺", "orb": 2, "color": AspectColor.SEMISEXTILE.value},
                45: {"name": "Semisquare", "symbol": "∠", "orb": 2, "color": AspectColor.SEMISQUARE.value},
                135: {"name": "Sesquisquare", "symbol": "⚼", "orb": 2, "color": AspectColor.SESQUISQUARE.value},
                150: {"name": "Quincunx", "symbol": "⚻", "orb": 3, "color": AspectColor.QUINCUNX.value},
                72: {"name": "Quintile", "symbol": "Q", "orb": 2, "color": AspectColor.QUINTILE.value},
                144: {"name": "Biquintile", "symbol": "bQ", "orb": 2, "color": AspectColor.BIQUINTILE.value},
            }
            ...
            for i, planet1 in enumerate(celestial_bodies):
                for planet2 in celestial_bodies[i + 1:]:
                    ...
                    # Check for aspects
                    for aspect_angle, aspect_info in aspect_definitions.items():
                        orb = aspect_info["orb"]
                        if abs(angle - aspect_angle) <= orb:
                            ...
                            applying = self._is_aspect_applying(angle, aspect_angle, speed1, speed2)
                            
                            aspects.append({
                                "planet1": planet1_data.get("name", planet1.capitalize()),
                                "planet2": planet2_data.get("name", planet2.capitalize()),
                                ...
                                "applying": applying,
                                "strength": self._calculate_aspect_strength(exactness, orb),
            ...
            logger.info(f"KERYKEION_SERVICE_ASPECTS_CALCULATED: {len(aspects)} aspects found")
            return aspects
    ...
    def _is_aspect_applying(self, current_angle: float, exact_angle: float, 
                          speed1: float, speed2: float) -> bool:
    ...
    def _calculate_aspect_strength(self, exactness: float, max_orb: float) -> str:
    ...
    def _get_aspect_interpretation(self, planet1: str, planet2: str, aspect: str) -> str:
        ...
        aspect_interpretations = interpretations.get(key, interpretations.get(reverse_key, {}))
        return aspect_interpretations.get(aspect, f"{aspect} между {planet1.capitalize()} и {planet2.capitalize()}")
    
    def calculate_arabic_parts_extended(self, subject: Any) -> Dict[str, Dict[str, Any]]:
        ...
        try:
            ...
            logger.info(f"KERYKEION_SERVICE_ARABIC_PARTS: {len(parts)} parts calculated")
            return parts
    ...
    def _analyze_chart_shape(self, planets_data: Dict[str, Any]) -> Dict[str, str]:
    ...
    def _calculate_element_distribution(self, planets_data: Dict[str, Any]) -> Dict[str, int]:
    ...
    def _calculate_quality_distribution(self, planets_data: Dict[str, Any]) -> Dict[str, int]:
    ...
    def _find_dominant_planets(self, planets_data: Dict[str, Any], 
                             aspects_data: List[Dict[str, Any]]) -> List[str]:
        ...
        for aspect in aspects_data:
            ...
            strength_points = {"Very Strong": 4, "Strong": 3, "Moderate": 2, "Weak": 1}
        ...
        sorted_planets = sorted(planet_scores.items(), key=lambda x: x[1], reverse=True)
    ...
    def generate_chart_svg(
        ...
        if not self.available or not KerykeionChartSVG:
            logger.error("KERYKEION_SERVICE_SVG: Chart generation not available")
            return None
    ...
    def calculate_compatibility_detailed(
        ...
        try:
            ...
            for lum1, lum2, connection in luminaries_aspects:
                if lum1 and lum2:
                    aspect = self._calculate_aspect_between_points(
                        lum1.get("longitude", 0),
                        lum2.get("longitude", 0)
                    )
                    if aspect:
                        compatibility["sun_moon_connections"].append({
                            "connection": connection,
                            "aspect": aspect,
                            "harmony_score": self._get_aspect_harmony_score(aspect),
                        })
            ...
            for rom1, rom2, connection in romance_aspects:
                if rom1 and rom2:
                    aspect = self._calculate_aspect_between_points(
                        rom1.get("longitude", 0),
                        rom2.get("longitude", 0)
                    )
                    if aspect:
                        compatibility["venus_mars_connections"].append({
                            "connection": connection,
                            "aspect": aspect,
                            "passion_score": self._get_aspect_passion_score(aspect),
                        })
    ...
    def get_service_capabilities(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "features": {
                ...
                "svg_generation": self.available and KerykeionChartSVG is not None,
                "house_systems": [system.value for system in HouseSystem] if self.available else [],
                "zodiac_types": [ztype.value for ztype in ZodiacType] if self.available else [],
                "aspect_colors": True,
                "detailed_compatibility": self.available,
                "element_analysis": True,
                "quality_analysis": True,
            },
            "limitations": [] if self.available else [
