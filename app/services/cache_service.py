...
class CacheService:
    ...
    def __init__(self, redis_url: Optional[str] = None):
        ...
        if REDIS_AVAILABLE and redis_url:
            try:
                ...
                logger.warning(
                    f"Failed to connect to Redis: {e}. Using memory cache."
                )
        ...
        try:
            if self.redis_client:
                ...
                await self.redis_client.setex(
                    key, expiry_seconds, serialized_value
                )
                return True
            else:
                # Use memory cache
    ...
    async def set_user_devices(
        self, user_id: int, devices: list, expiry_minutes: int = 30
    ) -> bool:
    ...
    async def get_analytics_result(
        self, user_id: int, analysis_type: str, period_days: int
    ) -> Optional[Dict[str, Any]]:
    ...
    async def set_automation_insights(
        self, user_id: int, insights: Dict[str, Any], expiry_hours: int = 6
    ) -> bool:
