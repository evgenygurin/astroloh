...
try:
    ...
    logger.warning(
        f"PROGRESSION_SERVICE_INIT: Kerykeion progressions not available - {e}"
    )
...
class ProgressionService:
    ...
    def _get_kerykeion_progressions(
        ...
        try:
            ...
            interpretation = self._create_progression_interpretation(
                progressed_planets, days_progressed, target_date
            )
    ...
    def get_solar_return(
        ...
        interpretation = self._interpret_solar_return(
            solar_positions, solar_houses, year
        )
    ...
    def get_lunar_return(
        ...
        lunar_positions = self.astro_calculator.calculate_planet_positions(
            new_moon_date, coordinates["latitude"], coordinates["longitude"]
        )
        ...
        interpretation = self._interpret_lunar_return(
            lunar_positions, lunar_houses, month
        )
    ...
    def _extract_progressed_planets(
        self, progressed_subject: Any
    ...
    def _create_progression_interpretation(
        ...
        progressed_sun = ProgressedPlanet(
            ...
            meaning=self._interpret_progressed_sun(
                prog_sun.get("sign", "Unknown"), current_age
            ),
        )
    ...
    def _interpret_progressed_sun(self, sign: str, age: int) -> str:
        ...
        return sun_interpretations.get(
            sign, f"В {age} лет развитие через энергию {sign}"
        )
    ...
    def _identify_key_changes(
        ...
        if not changes:
            changes.append(
                "Продолжается углубление и развитие в рамках текущих энергий"
            )

        return changes[:5]  # Ограничиваем 5 ключевыми изменениями

    def _identify_basic_key_changes(
    ...
    def _identify_general_trends(
        self, progressed_planets: Dict[str, Any], age: int
    ) -> List[str]:
    ...
    def _interpret_solar_return(
        ...
        sun_house = self._find_planet_house(
            solar_positions.get("Sun", {}), solar_houses
        )

        # Анализируем Луну в доме
        moon_house = self._find_planet_house(
            solar_positions.get("Moon", {}), solar_houses
        )
    ...
    def _find_planet_house(
        self, planet_data: Dict[str, Any], houses: Dict[int, Any]
    ) -> Optional[int]:
        """Находит дом планеты."""
    ...
    def _interpret_lunar_return(
        ...
        return {
            ...
            "emotional_house_focus": self._get_house_meaning(moon_house)
            if moon_house
            else "общие эмоции",
            "action_house_focus": self._get_house_meaning(sun_house)
            if sun_house
    ...
    def _identify_success_indicators(
        ...
        if jupiter:
            ...
            if jupiter_house:
                indicators.append(
                    f"Удача в сфере {self._get_house_meaning(jupiter_house)}"
                )
    ...
    def _get_solar_key_areas(
        ...
        emphasized_houses = sorted(
            house_emphasis.items(), key=lambda x: x[1], reverse=True
        )[:3]
    ...
    def _identify_solar_challenges(
        ...
        for planet, data in positions.items():
            if data.get("retrograde", False) and planet in [
                "Mercury",
                "Venus",
                "Mars",
            ]:
    ...
    def _get_lunar_advice(self, moon_sign: str, sun_sign: str) -> str:
        ...
        advice_combinations = {
            (
                "Рак",
                "Лев",
            ): "Сочетайте эмоциональную чуткость с творческим самовыражением",
            (
                "Скорпион",
                "Стрелец",
            ): "Глубокие трансформации откроют новые горизонты",
            (
                "Рыбы",
                "Овен",
            ): "Интуиция поможет в принятии решительных действий",
        }
    ...
    def _get_emotional_guidance(self, positions: Dict[str, Any]) -> List[str]:
        ...
        guidance_by_sign = {
            ...
            "Телец": [
                "Ищите стабильность",
                "Наслаждайтесь простыми удовольствиями",
            ],
