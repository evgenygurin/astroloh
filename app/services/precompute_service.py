...
class PrecomputeService:
    ...
    def __init__(self):
        ...
        logger.info(
            "PRECOMPUTE_SERVICE_INIT: Pre-computation service initialized"
        )
    ...
    async def start_background_precomputation(self):
        ...
        logger.info(
            "PRECOMPUTE_SERVICE_START: Background pre-computation started"
        )
    ...
    async def stop_background_precomputation(self):
        ...
        if self._precompute_task:
            ...
            try:
                ...
                pass
        logger.info(
            "PRECOMPUTE_SERVICE_STOP: Background pre-computation stopped"
        )
                ...
                for task_name, schedule in self.precompute_schedule.items():
                    if self._should_run_task(
                        ...
                        try:
                            ...
                            logger.info(
                                f"PRECOMPUTE_SERVICE_TASK_SUCCESS: {task_name}"
                            )
                            ...
                            logger.error(
                                f"PRECOMPUTE_SERVICE_TASK_ERROR: {task_name} - {e}"
                            )
    ...
    async def _precompute_daily_ephemeris(self):
        ...
        for i in range(7):  # Next 7 days
            ...
            cached_ephemeris = await astro_cache.get_daily_ephemeris(
                target_date
            )
            if cached_ephemeris:
                continue
        ...
        logger.info(
            f"PRECOMPUTE_EPHEMERIS_SUCCESS: {precomputed_count} days precomputed"
        )
    ...
    async def _precompute_popular_charts(self):
        ...
        for i in range(0, len(chart_requests), batch_size):
            ...
            if uncached_requests:
                results = await async_kerykeion.batch_calculate_charts(
                    uncached_requests, use_cache=True
                )
                precomputed_count += len(
        ...
        logger.info(
            f"PRECOMPUTE_CHARTS_SUCCESS: {precomputed_count} charts precomputed"
        )
    ...
    async def _precompute_lunar_phases(self):
        ...
        await astro_cache.set(
            "lunar_phases_30days", lunar_data, 86400 * 7
        )  # Cache for 7 days

        logger.info(
            "PRECOMPUTE_LUNAR_SUCCESS: 30 days of lunar phases precomputed"
        )
    ...
    async def _precompute_zodiac_compatibility(self):
        ...
        for i, sign1 in enumerate(self.popular_signs):
            for sign2 in self.popular_signs[
                i:
        ...
        for sign1, sign2 in popular_combinations[
            ...
            cached_compatibility = (
                await astro_cache.get_compatibility_analysis(
            ...
            if not chart1_data.get("error") and not chart2_data.get("error"):
                ...
                compatibility = (
                    await async_kerykeion.calculate_compatibility_detailed(
        ...
        logger.info(
            f"PRECOMPUTE_COMPATIBILITY_SUCCESS: {precomputed_count} combinations precomputed"
        )
    ...
    async def _precompute_transit_forecasts(self):
        ...
        await astro_cache.set(
            "popular_transits_forecast", forecast_data, 28800
        )  # 8 hours

        logger.info(
            "PRECOMPUTE_TRANSITS_SUCCESS: Transit forecasts precomputed"
        )
    ...
    async def manual_precompute_all(self) -> Dict[str, Any]:
        ...
        for task_name, task_func in tasks:
            op_id = performance_monitor.start_operation(
                "PrecomputeService", f"manual_{task_name}"
            )
    ...
    async def get_precompute_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "popular_data_counts": {
                ...
                "chart_combinations": len(self.popular_signs)
                * len(self.popular_locations),
            },
        }

    def _get_next_run_time(
        self, task_name: str, schedule: Dict[str, Any]
    ) -> datetime:
