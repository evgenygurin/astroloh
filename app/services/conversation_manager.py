...
class ConversationManager:
    ...
    async def process_conversation(
        ...
        enhanced_request = await self._enhance_request_with_context(
            processed_request, conversation
        )
        ...
        try:
            async with get_db_session_context() as db:
                ...
                if user:
                    ...
                    self.logger.info(
                        f"Loaded conversation history for user {conversation.user_id}"
                    )
    ...
    async def _enhance_request_with_context(
        ...
        if recent_intents:
            enhanced_entities["recent_context"] = [
                intent.value for intent in recent_intents[-3:]
            ]
    ...
    def _generate_adaptive_suggestions(
        self, conversation: ConversationContext, state: DialogState
    ) -> List[str]:
        ...
        if (
            "compatibility" not in preferred_topics
            and state != DialogState.EXPLORING_COMPATIBILITY
        ):
            suggestions.append("Совместимость")

        if (
            "natal_chart" not in preferred_topics
            and state != DialogState.DISCUSSING_NATAL_CHART
        ):
        ...
        if (
            "horoscope" not in preferred_topics
            and state != DialogState.PROVIDING_HOROSCOPE
        ):
    ...
    async def _update_user_preferences(
        self, conversation: ConversationContext, request: ProcessedRequest
    ) -> None:
        ...
        try:
            async with get_db_session_context() as db:
                ...
                if user:
                    # Обновляем предпочтения пользователя
                    preferences_json = {
                        "intent_frequency": conversation.preferences.get(
                            "intent_frequency", {}
                        ),
                        "interaction_style": conversation.preferences.get(
                            "interaction_style", "detailed"
                        ),
                    }
                    ...
                    self.logger.info(
                        f"Saved preferences for user {conversation.user_id}"
                    )
    ...
    def get_conversation_analytics(self, user_id: str) -> Dict[str, Any]:
        ...
        total_interactions = sum(
            conv.interaction_count for conv in user_conversations.values()
        )
        avg_personalization = sum(
            conv.personalization_level for conv in user_conversations.values()
        ...
        return {
            ...
            "most_active_session": max(
                user_conversations.values(), key=lambda c: c.interaction_count
            ).session_id,
        }
    ...
    async def get_conversation_context(
        ...
        user = await user_manager.get_user_by_yandex_id(db_session, user_id)
    ...
    def get_personalized_suggestions(self, intent: str, context) -> List[str]:
        ...
        return general_suggestions.get(
            intent, ["Гороскоп", "Совместимость", "Натальная карта"]
        )
