...
class SynastryService:
    ...
    def __init__(self, astrology_calculator: AstrologyCalculator):
        ...
        logger.info(
            f"SYNASTRY_SERVICE_KERYKEION: Available = {self.kerykeion_service.is_available()}"
        )
    ...
    async def calculate_advanced_synastry(
        ...
        """
        logger.info(
            f"SYNASTRY_ADVANCED_START: {person1.name} + {person2.name}"
        )

        try:
            if not self.kerykeion_service.is_available():
                logger.warning(
                    "SYNASTRY_ADVANCED: Fallback to basic calculator"
                )
                ...
                return {
                    ...
                    "composite_chart": {
                        "midpoints": basic.composite_midpoints or {}
                    },
                    "karmic_connections": {
                        "connections": [],
                        "strength": "unknown",
            ...
            birth_time1 = person1.birth_time or person1.birth_date.replace(
                hour=12, minute=0
            )
            birth_time2 = person2.birth_time or person2.birth_date.replace(
                hour=12, minute=0
            )
            ...
            if "error" in chart1_data or "error" in chart2_data:
                logger.error(
                    "SYNASTRY_ADVANCED_CHART_ERROR: Failed to create charts"
                )
            ...
            composite_chart = self._calculate_composite_midpoints(
                chart1_data.get("planets", {}), chart2_data.get("planets", {})
            )
            ...
            result = {
                "partner1": {"name": person1.name, "chart_data": chart1_data},
                "partner2": {"name": person2.name, "chart_data": chart2_data},
            ...
            logger.info(
                f"SYNASTRY_ADVANCED_SUCCESS: Generated advanced synastry for {person1.name} + {person2.name}"
            )
            return result
    ...
    def _analyze_house_overlays(
        ...
        try:
            ...
            for planet_name, planet_data in planets1.items():
                ...
                house_number = self._find_house_for_longitude(
                    planet_longitude, houses2
                )

                if house_number:
                    if house_number not in overlays:
                        overlays[house_number] = {
                            "planets": [],
                            "meanings": [],
                        }
            ...
            for house_num, overlay_data in overlays.items():
                overlay_data["meanings"] = self._get_house_overlay_meanings(
                    house_num, overlay_data["planets"]
                )
    ...
    def _find_house_for_longitude(
        self, longitude: float, houses: Dict[int, Any]
    ) -> Optional[int]:
    ...
    def _get_house_overlay_meanings(
        self, house_num: int, planets: List[Dict]
    ) -> List[str]:
    ...
    def _generate_synastry_interpretation(
