...
class YandexAdapter(PlatformAdapter):
    ...
    def to_universal_request(
        self, yandex_request: YandexRequestModel
    ) -> UniversalRequest:
        ...
        try:
            ...
            user_context = {
                ...
                "interfaces": yandex_request.session.interfaces.dict()
                if yandex_request.session.interfaces
                else {},
                "nlu": yandex_request.request.nlu.dict()
                if yandex_request.request.nlu
                else {},
    ...
    def from_universal_response(
        ...
        try:
            ...
            if universal_response.buttons:
                for button in universal_response.buttons[:5]:  # Yandex limit
                    yandex_button = YandexButton(
                        ...
                        or {"action": button.title.lower().replace(" ", "_")},
                        url=button.url,
                    )
            ...
            yandex_response = YandexResponse(
                ...
                tts=universal_response.tts
                or self._add_tts_pauses(universal_response.text),
