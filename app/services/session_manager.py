...
class SessionManager:
    ...
    def get_user_context(self, session: YandexSession) -> UserContext:
        ...
        logger.info(f"SESSION_GET_CONTEXT_START: session_key={session_key}, user_id={session.user_id}")

        if session_key not in self._sessions:
            # Инициализируем новую сессию
            logger.info(f"SESSION_NEW_USER: initializing_new_session for {session_key}")
            self._initialize_new_session(session)
            context = UserContext(user_id=session.user_id)
            logger.debug(f"SESSION_NEW_CONTEXT_CREATED: user_id={session.user_id}")
            return context

        session_data = self._sessions[session_key]
        logger.debug(f"SESSION_EXISTING_FOUND: message_count={session_data.get('message_count', 0)}")

        # Проверяем таймаут сессии
        if self._is_session_expired(session_data):
            logger.warning(f"SESSION_EXPIRED: cleaning_up session_key={session_key}")
            self._cleanup_expired_session(session_key)
            self._initialize_new_session(session)
            context = UserContext(user_id=session.user_id)
            logger.info(f"SESSION_EXPIRED_RENEWED: new_context_created for {session.user_id}")
            return context
        ...
        logger.debug(f"SESSION_ACTIVITY_UPDATED: message_count={old_message_count} -> {session_data['message_count']}")
        ...
        if self._is_conversation_stalled(session_data):
            logger.warning(f"SESSION_CONVERSATION_STALLED: resetting_awaiting_data for {session_key}")
            context.awaiting_data = None
            context.conversation_step = 0

        logger.info(f"SESSION_GET_CONTEXT_SUCCESS: awaiting_data={context.awaiting_data}, step={context.conversation_step}")
        return context
    ...
    def update_user_context(self, session: YandexSession, context: UserContext) -> None:
        ...
        logger.info(f"SESSION_UPDATE_CONTEXT_START: session_key={session_key}, intent={context.intent.value if context.intent else None}")

        # Получаем существующие данные или создаем новые
        existing_data = self._sessions.get(session_key, {})
        logger.debug(f"SESSION_UPDATE_EXISTING: has_existing_data={bool(existing_data)}, existing_interactions={existing_data.get('successful_interactions', 0)}")
        ...
        if not context.awaiting_data:
            ...
            if existing_data.get("awaiting_data"):
                ...
                logger.info(f"SESSION_INTERACTION_COMPLETED: successful_interactions={session_data['successful_interactions']}")
        elif context.awaiting_data and not existing_data.get("awaiting_data_since"):
            logger.debug(f"SESSION_AWAITING_DATA_SET: awaiting={context.awaiting_data}, since={now}")

        self._sessions[session_key] = session_data
        logger.info(f"SESSION_UPDATE_CONTEXT_SUCCESS: step={context.conversation_step}, awaiting={context.awaiting_data}")
    ...
    def clear_user_context(self, session: YandexSession) -> None:
        ...
        if session_key in self._sessions:
            ...
            logger.info(f"SESSION_CLEARED: session_key={session_key}, had_interactions={interactions}")
        else:
            logger.warning(f"SESSION_CLEAR_NOT_FOUND: session_key={session_key}")
    ...
    def set_awaiting_data(
        ...
        logger.info(f"SESSION_SET_AWAITING_DATA: data_type={data_type}, intent={intent.value if intent else None}")
        ...
        logger.debug(f"SESSION_CONVERSATION_STEP: {old_step} -> {context.conversation_step}")
    ...
    def clear_awaiting_data(self, session: YandexSession, context: UserContext) -> None:
        ...
        logger.info(f"SESSION_CLEAR_AWAITING_DATA: was_awaiting={context.awaiting_data}")
        
        context.awaiting_data = None
        old_step = context.conversation_step
        context.conversation_step += 1
        logger.debug(f"SESSION_CLEAR_CONVERSATION_STEP: {old_step} -> {context.conversation_step}")
    ...
    def cleanup_expired_sessions(self) -> int:
        ...
        logger.info(f"SESSION_CLEANUP_START: total_sessions={len(self._sessions)}")
        ...
        for session_key, session_data in self._sessions.items():
            if self._is_session_expired(session_data):
                ...
                logger.debug(f"SESSION_FOUND_EXPIRED: session_key={session_key}")
            elif self._is_conversation_stalled(session_data):
                stalled_conversations += 1
                logger.warning(f"SESSION_FOUND_STALLED: session_key={session_key}")
        ...
        logger.info(f"SESSION_CLEANUP_COMPLETE: expired={len(expired_sessions)}, stalled={stalled_conversations}, remaining={len(self._sessions)}")
    ...
    def _initialize_new_session(self, session: YandexSession) -> None:
        ...
        logger.debug(f"SESSION_INITIALIZED: user_id={session.user_id}, total_sessions={self._session_stats['total_sessions']}")
    ...
    def _cleanup_expired_session(self, session_key: str) -> None:
        ...
        if session_key in self._sessions:
            ...
            if interactions > 0:
                ...
                logger.debug(f"SESSION_COMPLETED_FLOW: session_key={session_key}, interactions={interactions}")
            del self._sessions[session_key]
    ...
    def get_session_analytics(self) -> Dict[str, Any]:
        ...
        logger.debug(f"SESSION_ANALYTICS_RESULT: active={active_count}, avg_messages={analytics['average_messages_per_session']}, stalled={stalled_count}")
        return analytics
