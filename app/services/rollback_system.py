...
class RollbackSystem:
    ...
    async def analyze_rollback_need(self) -> Optional[RollbackPlan]:
        ...
        try:
            logger.debug("ROLLBACK_ANALYSIS: Analyzing system for rollback need")
            
            # Check circuit breaker
            if await self._is_circuit_breaker_active():
                logger.warning("ROLLBACK_CIRCUIT_BREAKER: Circuit breaker active, skipping analysis")
                return None
            ...
            for rule_name, rule in self.rollback_rules.items():
                if await self._evaluate_rollback_condition(rule, health_checks, dashboard):
                    logger.warning(f"ROLLBACK_CONDITION_MET: Rule '{rule_name}' triggered")
                    ...
                    if rollback_plan and rule.get("auto_execute", False):
                        logger.critical(f"ROLLBACK_AUTO_TRIGGER: Automatic rollback planned for rule '{rule_name}'")
    ...
    async def _create_rollback_plan(
        ...
        try:
            ...
            affected_features = await self._identify_affected_features(trigger, rule_name)
            ...
            if strategy == RollbackStrategy.IMMEDIATE:
                rollback_steps = [
                    {"action": "disable_all_features", "features": affected_features, "duration": 10},
                ...
                rollback_steps = [
                    {"action": "reduce_rollout_percentage", "features": affected_features, "target_percentage": 50, "duration": 60},
                    {"action": "reduce_rollout_percentage", "features": affected_features, "target_percentage": 20, "duration": 60},
                    {"action": "reduce_rollout_percentage", "features": affected_features, "target_percentage": 5, "duration": 60},
                    {"action": "disable_features_if_needed", "features": affected_features, "duration": 30},
                ]
                expected_duration = 210
            
            elif strategy == RollbackStrategy.FEATURE_SPECIFIC:
                rollback_steps = [
                    {"action": "disable_specific_features", "features": affected_features, "duration": 20},
                    {"action": "enable_fallbacks", "features": affected_features, "duration": 10},
                ...
                rollback_steps = [
                    ...
                    {"action": "disable_all_kerykeion_features", "duration": 20},
            ...
            plan = RollbackPlan(
                ...
                user_notification_required=(strategy in [RollbackStrategy.FULL_SYSTEM, RollbackStrategy.IMMEDIATE]),
                backup_data=self.system_state_backup,
            )
            
            self.active_rollbacks[rollback_id] = plan
            
            logger.info(f"ROLLBACK_PLAN_CREATED: {rollback_id} with {len(rollback_steps)} steps")
            return plan
    ...
    async def _identify_affected_features(self, trigger: RollbackTrigger, rule_name: str) -> List[str]:
        ...
        if trigger == RollbackTrigger.HIGH_FALLBACK_RATE:
            ...
            return [f for f in all_features if "kerykeion" in f or "enhanced" in f or "ai" in f]
    ...
    async def execute_rollback(self, plan: RollbackPlan) -> RollbackEvent:
        ...
        try:
            logger.critical(f"ROLLBACK_EXECUTION_START: Executing rollback {plan.rollback_id}")
            ...
            for i, step in enumerate(plan.rollback_steps):
                try:
                    logger.info(f"ROLLBACK_STEP: {i+1}/{len(plan.rollback_steps)} - {step['action']}")
                    ...
                    logger.error(f"ROLLBACK_STEP_ERROR: Step {i+1} failed: {step_error}")
                    rollback_event.rollback_data[f"step_{i+1}_error"] = str(step_error)
            ...
            if success:
                logger.info(f"ROLLBACK_EXECUTION_SUCCESS: Rollback {plan.rollback_id} completed in {recovery_time:.1f}s")
            else:
                logger.error(f"ROLLBACK_EXECUTION_PARTIAL: Rollback {plan.rollback_id} partially successful")
            ...
            rollback_event.recovery_time = (datetime.now() - start_time).total_seconds()
            self.rollback_history.append(rollback_event)
            return rollback_event
    
    async def _execute_rollback_step(self, step: Dict[str, Any], rollback_event: RollbackEvent):
        ...
        if action == "disable_all_features":
            ...
            rollback_event.rollback_data["disabled_features"] = step["features"]
        
        elif action == "disable_specific_features":
            for feature in step["features"]:
                feature_flags.emergency_rollback(feature)
            rollback_event.rollback_data["disabled_specific"] = step["features"]
        
        elif action == "reduce_rollout_percentage":
            target_percentage = step["target_percentage"]
            for feature in step["features"]:
                feature_flags.update_feature_flag(feature, rollout_percentage=target_percentage)
            rollback_event.rollback_data["reduced_to_percentage"] = target_percentage
            ...
            rollback_event.rollback_data["fallbacks_enabled"] = step["features"]
            ...
            dashboard = await deployment_monitor.get_deployment_dashboard()
            rollback_event.rollback_data["recovery_status"] = dashboard.get("overall_health", {})
    ...
    async def _verify_rollback_success(self, plan: RollbackPlan, rollback_event: RollbackEvent) -> bool:
    ...
    async def _is_circuit_breaker_active(self) -> bool:
        ...
        if (self.circuit_breaker["last_rollback_time"] and 
            now - self.circuit_breaker["last_rollback_time"] > timedelta(hours=1)):
            self.circuit_breaker["rollback_count"] = 0
        
        # Check if we've exceeded the limit
        if self.circuit_breaker["rollback_count"] >= self.circuit_breaker["max_rollbacks_per_hour"]:
            # Check cooldown period
            if (self.circuit_breaker["last_rollback_time"] and
                now - self.circuit_breaker["last_rollback_time"] < timedelta(minutes=self.circuit_breaker["cooldown_minutes"])):
                return True
    ...
    async def manual_rollback(
        ...
        logger.warning(f"ROLLBACK_MANUAL: Manual rollback requested for {len(features)} features")
        ...
        rollback_steps = [
            ...
            {"action": "disable_specific_features", "features": features, "duration": 20},
            {"action": "clear_cache", "duration": 5},
    ...
    def get_rollback_statistics(self) -> Dict[str, Any]:
        ...
        successful_rollbacks = sum(1 for rb in self.rollback_history if rb.success)
        success_rate = (successful_rollbacks / total_rollbacks) * 100
        
        recovery_times = [rb.recovery_time for rb in self.rollback_history if rb.recovery_time]
        ...
        return {
            ...
            "circuit_breaker_status": "active" if await self._is_circuit_breaker_active() else "inactive",
