...
class IoTAnalyticsService:
    ...
    async def analyze_energy_consumption(
        ...
        try:
            ...
            if not energy_data:
                return {
                    "success": False,
                    "message": "No energy consumption data available",
                }
            ...
            for data_point in energy_data:
                device_consumption[data_point.device_id].append(
                    data_point.value
                )
            ...
            avg_daily_consumption = (
                total_consumption / len(daily_consumption)
            ...
            peak_hours = await self._analyze_peak_consumption_hours(
                energy_data
            )

            # Device efficiency analysis
            device_efficiency = await self._analyze_device_efficiency(
                device_consumption
            )
    ...
    async def _calculate_energy_lunar_correlation(
        ...
        try:
            ...
            if phase_averages:
                highest_phase = max(
                    phase_averages.keys(),
                    key=lambda x: phase_averages[x]["average"],
                )
                lowest_phase = min(
                    phase_averages.keys(),
                    key=lambda x: phase_averages[x]["average"],
                ...
                return {
                    ...
                    "variation": round(
                        phase_averages[highest_phase]["average"]
                        - phase_averages[lowest_phase]["average"],
                        2,
                    ),
                }
    ...
    async def _analyze_device_efficiency(
        ...
        for device_id, consumptions in device_consumption.items():
            ...
            consistency = (
                1 / (statistics.stdev(consumptions) + 1)
    ...
    async def _generate_energy_recommendations(
        self,
        lunar_correlation: Dict[str, Any],
        ...
        if device_efficiency:
            inefficient_devices = [
                device_id
                for device_id, data in device_efficiency.items()
    ...
    async def analyze_wellness_correlations(
        ...
        try:
            ...
            activity_analysis = await self._analyze_activity_patterns(
                wellness_data
            )
            lunar_sync_analysis = await self._analyze_lunar_synchronization(
                wellness_data
            )
            ...
            return {
                ...
                "overall_wellness_score": await self._calculate_wellness_score(
                    wellness_data
                ),
            }
    ...
    async def _analyze_sleep_patterns(
        self, wellness_data: List[WearableData]
    ) -> Dict[str, Any]:
        ...
        return {
            ...
            "consistency": round(
                1 / (statistics.stdev(sleep_scores) + 0.01), 2
            )
            if len(sleep_scores) > 1
            else 1,
            "trend": "improving"
            if sleep_scores[-5:] > sleep_scores[:5]
            else "stable",
        }

    async def _analyze_mood_patterns(
        self, wellness_data: List[WearableData]
    ) -> Dict[str, Any]:
        """Analyze mood score patterns."""
        mood_scores = [
            d.mood_score for d in wellness_data if d.mood_score is not None
        ]
        ...
        return {
            ...
            "volatility": round(statistics.stdev(mood_scores), 2)
    ...
    async def _analyze_activity_patterns(
        ...
        activity_scores = [
            d.activity_level
            for d in wellness_data
            if d.activity_level is not None
        ]
    ...
    async def _analyze_lunar_synchronization(
        self, wellness_data: List[WearableData]
    ) -> Dict[str, Any]:
        ...
        lunar_correlations = [
            d.lunar_correlation
            for d in wellness_data
            if d.lunar_correlation is not None
        ]
        ...
        return {
            ...
            "highly_synchronized_days": len(
                [c for c in lunar_correlations if c > 0.7]
            ),
            "total_days": len(lunar_correlations),
        }

    async def _calculate_wellness_score(
        self, wellness_data: List[WearableData]
    ) -> float:
        ...
        for data in wellness_data:
            ...
            if data.stress_level is not None:
                scores.append(
                    (1 - data.stress_level) * 0.15
                )  # Lower stress is better
                weight_sum += 0.15

            if data.lunar_correlation is not None:
                scores.append(
                    abs(data.lunar_correlation) * 0.1
                )  # Any correlation is good
                weight_sum += 0.1
    ...
    async def generate_automation_insights(
        ...
        try:
            ...
            successful_automations = len(
                [a for a in automations if a.execution_count > 0]
            )

            # Most and least used automations
            most_used = (
                max(automations, key=lambda x: x.execution_count)
                if automations
                else None
            )
            least_used = (
                min(automations, key=lambda x: x.execution_count)
                if automations
                else None
            )
            ...
            return {
                ...
                "most_used": {
                    ...
                    "executions": most_used.execution_count
                    if most_used
                    else 0,
                }
                if most_used
                ...
                "least_used": {
                    ...
                    "executions": least_used.execution_count
                    if least_used
                    else 0,
                }
                if least_used
    ...
    async def _generate_automation_recommendations(
        ...
        if trigger_stats:
            most_popular_trigger = max(
                trigger_stats.keys(), key=lambda x: trigger_stats[x]
            )
            recommendations.append(
        ...
        lunar_automations = [
            a for a in automations if "lunar" in a.trigger_type.lower()
        ]
    ...
    async def generate_monthly_report(self, user_id: int) -> Dict[str, Any]:
        ...
        try:
            ...
            energy_analysis = await self.analyze_energy_consumption(
                user_id, 30
            )
            wellness_analysis = await self.analyze_wellness_correlations(
                user_id, 30
            )
            automation_insights = await self.generate_automation_insights(
                user_id
            )

            # Device status summary
            device_query = select(IoTDevice).where(
                IoTDevice.user_id == user_id
            )
            ...
            for device in devices:
                ...
                device_summary["device_types"][device_type] = (
                    device_summary["device_types"].get(device_type, 0) + 1
                )
    ...
    async def _calculate_overall_smart_home_score(
        ...
        if energy.get("success"):
            ...
            if "variation" in lunar_correlation:
                ...
                energy_score = max(
                    0, 1 - (lunar_correlation["variation"] / 100)
                )
