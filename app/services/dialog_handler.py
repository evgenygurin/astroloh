...
class DialogHandler:
    ...
    async def handle_request(self, request: YandexRequestModel) -> YandexResponseModel:
        ...
        try:
            ...
            response = await self._generate_contextual_response(
                dialog_state, response_context, processed_request, request.session
            )
            ...
            error_context, recovery_response = self.error_recovery_manager.handle_error(
                e, {
                    "user_id": request.session.user_id,
                    "session_id": request.session.session_id,
        ...
        self.error_handler.log_request_processing(
            ...
            success=True,
            additional_context={
                "personalization_level": response_context.get("interaction_stats", {}).get("personalization_level", 0),
                "dialog_state": dialog_state.value if 'dialog_state' in locals() else "unknown"
            }
        )
    ...
    async def _generate_contextual_response(
        ...
        if dialog_state == DialogState.COLLECTING_BIRTH_DATA:
            return await self._handle_birth_data_collection(response_context, processed_request, session)
        elif dialog_state == DialogState.COLLECTING_PARTNER_DATA:
            return await self._handle_partner_data_collection(response_context, processed_request, session)
        elif dialog_state == DialogState.PROVIDING_HOROSCOPE:
            return await self._handle_personalized_horoscope(response_context, processed_request, session)
        elif dialog_state == DialogState.EXPLORING_COMPATIBILITY:
            return await self._handle_compatibility_exploration(response_context, processed_request, session)
        elif dialog_state == DialogState.DISCUSSING_NATAL_CHART:
            return await self._handle_natal_discussion(response_context, processed_request, session)
        elif dialog_state == DialogState.LUNAR_GUIDANCE:
            return await self._handle_lunar_guidance(response_context, processed_request, session)
        elif dialog_state == DialogState.GIVING_ADVICE:
            return await self._handle_personalized_advice(response_context, processed_request, session)
        elif dialog_state == DialogState.CLARIFYING_REQUEST:
            return await self._handle_clarification(response_context, processed_request, session)
        elif dialog_state == DialogState.ERROR_RECOVERY:
            return await self._handle_error_recovery(response_context, processed_request, session)
        else:
    ...
    async def _handle_birth_data_collection(
        self, response_context: Dict[str, Any], processed_request: ProcessedRequest, session
    ) -> Any:
        ...
        if entities.get("dates") or flow_context.get("birth_date"):
            return await self._handle_horoscope(entities, processed_request.user_context, session)
    ...
    async def _handle_partner_data_collection(
        ...
        return await self._handle_compatibility(processed_request.entities, processed_request.user_context, session)
    ...
    async def _handle_personalized_horoscope(
        ...
        if response_context.get("can_provide_service"):
            ...
            result = await self._handle_horoscope(entities, processed_request.user_context, session)
            
            # Добавляем контекстные предложения
            if hasattr(result, 'buttons') and response_context.get("contextual_suggestions"):
    ...
    async def _handle_natal_discussion(
        ...
        return await self._handle_natal_chart(processed_request.entities, processed_request.user_context, session)

    async def _handle_lunar_guidance(
        self, response_context: Dict[str, Any], processed_request: ProcessedRequest, session
    ) -> Any:
        """Обрабатывает лунное руководство с персонализацией."""
        return await self._handle_lunar_calendar(processed_request.user_context, session)
    ...
    async def _handle_error_recovery(
        ...
        return self.response_formatter.format_error_recovery_response(unique_suggestions)
