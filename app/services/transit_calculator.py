...
class TransitCalculator:
    ...
    def get_enhanced_current_transits(
        ...
        """
        self.logger.info(
            "TRANSIT_CALCULATOR_ENHANCED_CURRENT: Using enhanced transit service"
        )
        ...
        if enhanced_result.get("source") == "basic" or not enhanced_result:
            self.logger.warning(
                "TRANSIT_CALCULATOR_ENHANCED_FALLBACK: Using legacy method"
            )
    ...
    def get_period_forecast(
        ...
        """
        self.logger.info(
            f"TRANSIT_CALCULATOR_PERIOD_FORECAST: {days} days forecast"
        )

        return self.transit_service.get_period_forecast(
            natal_chart, days, start_date
        )
    ...
    def get_important_transits(
        ...
        """
        self.logger.info(
            "TRANSIT_CALCULATOR_IMPORTANT: Analyzing major transits"
        )

        return self.transit_service.get_important_transits(
            natal_chart, lookback_days, lookahead_days
        )

    def get_secondary_progressions(
        self, natal_chart: Dict[str, Any], target_date: Optional[date] = None
    ) -> Dict[str, Any]:
        """
        ...
        """
        self.logger.info(
            "TRANSIT_CALCULATOR_PROGRESSIONS: Calculating secondary progressions"
        )

        return self.progression_service.get_secondary_progressions(
            natal_chart, target_date
        )
    ...
    def get_enhanced_solar_return(
        ...
        enhanced_result = self.progression_service.get_solar_return(
            natal_chart, year, location
        )
    ...
    def get_enhanced_lunar_return(
        ...
        """
        self.logger.info(
            f"TRANSIT_CALCULATOR_LUNAR_RETURN_ENHANCED: {month}/{year}"
        )

        # Используем новый Progression Service
        enhanced_result = self.progression_service.get_lunar_return(
            natal_chart, month, year, location
        )
        ...
        if not enhanced_result:
            ...
            return self.calculate_lunar_return(
                birth_date, month, year, location
            )

        return enhanced_result

    def get_comprehensive_transit_analysis(
        self, natal_chart: Dict[str, Any], analysis_period_days: int = 30
    ) -> Dict[str, Any]:
        """
        ...
        period_forecast = self.get_period_forecast(
            natal_chart, analysis_period_days
        )
        important_transits = self.get_important_transits(natal_chart)
        progressions = self.get_secondary_progressions(natal_chart)
        solar_return = self.get_enhanced_solar_return(
            natal_chart, current_year
        )
        lunar_return = self.get_enhanced_lunar_return(
            natal_chart, current_month, current_year
        )
        ...
        comprehensive_analysis = {
            ...
            "life_phase_analysis": self._analyze_current_life_phase(
                progressions, solar_return
            ),
        ...
        self.logger.info(
            "TRANSIT_CALCULATOR_COMPREHENSIVE_SUCCESS: Analysis complete"
        )
    ...
    def _analyze_current_life_phase(
        self, progressions: Dict[str, Any], solar_return: Dict[str, Any]
    ) -> Dict[str, str]:
        ...
        solar_theme = solar_return.get("interpretation", {}).get(
            "year_theme", "Год развития"
        )
        ...
        return {
            ...
            "development_phase": self._determine_development_phase(
                current_age
            ),
    ...
    def _create_spiritual_guidance(
        ...
        if not spiritual_messages:
            spiritual_messages.append(
                "Каждый момент жизни несет возможности для роста и развития сознания"
            )
    ...
    def _create_executive_summary(
        ...
        if important_transits.get("important_transits"):
            summary_parts.append(
                "Долгосрочно ожидаются значительные астрологические влияния"
            )
