...
class TransitCalculator:
        ...
        for aspect_angle, orb in self.transit_orbs.items():
            if abs(angle - aspect_angle) <= orb:
                ...
                aspects.append({
                    ...
                    "exact_angle": angle,
                    "influence": influence,
                    "strength": self._calculate_aspect_strength(exactness, aspect_angle),
    ...
    def _get_transit_influence(
        ...
        base_influence = general_influences.get(transit_planet, "Влияние на жизненные процессы")
    ...
    def _create_transit_summary(self, active_transits: List[Dict[str, Any]]) -> str:
        ...
        strong_transits = [t for t in active_transits if t["strength"] in ["сильный", "очень сильный"]]
        ...
        harmonious = [t for t in strong_transits if t["nature"] in ["гармония", "поток"]]
        challenging = [t for t in strong_transits if t["nature"] in ["напряжение", "противостояние"]]
    ...
    def _analyze_house_emphasis(
        ...
        for planet, planet_data in positions.items():
            ...
            for house_num in range(1, 13):
                house_start = houses.get(house_num, {}).get("cusp_longitude", (house_num - 1) * 30)
                house_end = houses.get(house_num % 12 + 1, {}).get("cusp_longitude", house_num * 30)
    ...
    def _get_key_life_areas(self, house_distribution: Dict[int, int]) -> List[str]:
        ...
        emphasized_houses = sorted(house_distribution.items(), key=lambda x: x[1], reverse=True)[:3]
    ...
    def _identify_solar_challenges(self, positions: Dict[str, Dict[str, Any]]) -> List[str]:
    ...
    def _identify_solar_opportunities(self, positions: Dict[str, Dict[str, Any]]) -> List[str]:
        ...
        for aspect in aspects:
            if aspect["aspect"] in ["Трин", "Секстиль"] and aspect["orb"] <= 5:
                opportunity = f"Гармония между {aspect['planet1']} и {aspect['planet2']}"
    ...
    def _interpret_lunar_return(self, positions: Dict[str, Dict[str, Any]]) -> Dict[str, str]:
