...
class StartupManager:
    ...
    async def initialize_performance_systems(
        ...
        try:
            ...
            await self._initialize_cache_service(
                initialization_results, redis_url
            )

            # 3. Initialize performance monitoring
            await self._initialize_performance_monitor(
                initialization_results, enable_background_monitoring
            )

            # 4. Initialize pre-computation service
            await self._initialize_precompute_service(
                initialization_results, enable_precomputation
            )
            ...
            logger.info(
                f"STARTUP_MANAGER_COMPLETE: All systems initialized in {self.startup_stats['duration_seconds']:.2f}s"
            )
    ...
    async def _initialize_async_kerykeion(self, results: Dict[str, Any]):
        ...
        try:
            logger.info(
                "STARTUP_MANAGER_KERYKEION: Initializing async Kerykeion service"
            )
            ...
            results["async_kerykeion"] = {"status": "error", "error": str(e)}
            self.startup_stats["services_failed"] += 1
            self.startup_errors.append(f"Async Kerykeion: {str(e)}")

    async def _initialize_cache_service(
        self, results: Dict[str, Any], redis_url: Optional[str]
    ):
        ...
        try:
            ...
            if redis_url:
                ...
                logger.info(
                    f"STARTUP_MANAGER_CACHE_REDIS: Redis configured at {redis_url}"
                )
            ...
            results["cache_service"] = {"status": "error", "error": str(e)}
            self.startup_stats["services_failed"] += 1
            self.startup_errors.append(f"Cache Service: {str(e)}")

    async def _initialize_performance_monitor(
        self, results: Dict[str, Any], enable_background: bool
    ):
        """Initialize performance monitoring."""
        try:
            logger.info(
                "STARTUP_MANAGER_MONITOR: Initializing performance monitor"
            )
            ...
            op_id = performance_monitor.start_operation(
                "StartupManager", "test_operation"
            )
            await asyncio.sleep(0.1)  # Simulate some work
            performance_monitor.end_operation(op_id, success=True)

            monitor_stats = performance_monitor.get_service_statistics(
                "StartupManager"
            )
    ...
    async def _initialize_precompute_service(
        self, results: Dict[str, Any], enable_precomputation: bool
    ):
        """Initialize pre-computation service."""
        try:
            logger.info(
                "STARTUP_MANAGER_PRECOMPUTE: Initializing pre-computation service"
            )
            ...
            precompute_status = (
                await precompute_service.get_precompute_status()
            )
    ...
    async def _warm_up_cache(self, results: Dict[str, Any]):
        ...
        try:
            ...
            manual_precompute = (
                await precompute_service.manual_precompute_all()
            )
            ...
            results["cache_warmup"] = {"status": "error", "error": str(e)}
    ...
    async def _run_startup_diagnostics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            diagnostics["response_times"]["cache_ms"] = round(
                cache_response_time, 2
            )

            # Test async kerykeion availability
            start_time = time.time()
            await async_kerykeion.get_performance_stats()
            kerykeion_response_time = (time.time() - start_time) * 1000

            diagnostics["response_times"]["kerykeion_ms"] = round(
                kerykeion_response_time, 2
            )
            ...
            logger.info(
                f"STARTUP_MANAGER_DIAGNOSTICS_SUCCESS: System health: {diagnostics['system_health']}"
            )
    ...
    async def shutdown_performance_systems(self) -> Dict[str, Any]:
        ...
        logger.info(
            "STARTUP_MANAGER_SHUTDOWN: Shutting down performance systems"
        )
    ...
    async def get_system_status(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if self.startup_completed:
                ...
                status[
                    "performance_report"
                ] = performance_monitor.get_performance_report(24)
