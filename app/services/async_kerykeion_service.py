...
class AsyncKerykeionService:
    ...
    def __init__(self, max_workers: int = 4):
        ...
        logger.info(
            f"ASYNC_KERYKEION_INIT: Service initialized with {max_workers} workers"
        )
    ...
    def _generate_chart_id(
        ...
        data = (
            f"{birth_datetime.isoformat()}_{latitude}_{longitude}_{timezone}"
        )
    ...
    async def get_full_natal_chart_data(
        ...
        if use_cache:
            ...
            if cached_result:
                ...
                logger.info(
                    f"ASYNC_KERYKEION_NATAL_CACHED: {name} in {elapsed:.3f}s"
                )
        ...
        try:
            ...
            if elapsed > 2.0:  # Slow calculation threshold
                ...
                logger.warning(
                    f"ASYNC_KERYKEION_SLOW_CALCULATION: {elapsed:.3f}s for {name}"
                )
            ...
            logger.info(
                f"ASYNC_KERYKEION_NATAL_SUCCESS: {name} in {elapsed:.3f}s"
            )
            return result
    ...
    async def calculate_compatibility_detailed(
        ...
        person1_id = person1_data.get("subject_info", {}).get(
            "birth_datetime", "unknown1"
        )
        person2_id = person2_data.get("subject_info", {}).get(
            "birth_datetime", "unknown2"
        )
        ...
        if use_cache:
            ...
            if cached_result:
                ...
                self._update_average_time(elapsed)
                logger.info(
                    f"ASYNC_KERYKEION_COMPATIBILITY_CACHED: {elapsed:.3f}s"
                )
                return cached_result
        ...
        try:
            ...
            if elapsed > 3.0:  # Compatibility calculations can be slower
                self.performance_stats["slow_calculations"] += 1
                logger.warning(
                    f"ASYNC_KERYKEION_COMPATIBILITY_SLOW: {elapsed:.3f}s"
                )
            ...
            logger.info(
                f"ASYNC_KERYKEION_COMPATIBILITY_SUCCESS: {elapsed:.3f}s"
            )
            return result
            ...
            return {
                "error": f"Async compatibility calculation failed: {str(e)}"
            }

    async def calculate_arabic_parts_extended(
        self, natal_chart_data: Dict[str, Any], use_cache: bool = True
    ) -> Dict[str, Any]:
        ...
        chart_id = self._generate_chart_id(
            birth_datetime=datetime.fromisoformat(
                natal_chart_data.get("subject_info", {}).get(
                    "birth_datetime", "2000-01-01T00:00:00"
                )
            ),
            latitude=natal_chart_data.get("subject_info", {})
            .get("coordinates", {})
            .get("latitude", 0),
            longitude=natal_chart_data.get("subject_info", {})
            .get("coordinates", {})
            .get("longitude", 0),
        )
        ...
        if use_cache:
            cached_result = await astro_cache.get_arabic_parts(
                natal_chart_id=chart_id, include_extended=True
            )
            ...
            if cached_result:
                ...
                self._update_average_time(elapsed)
                logger.info(
                    f"ASYNC_KERYKEION_ARABIC_PARTS_CACHED: {elapsed:.3f}s"
                )
                return cached_result
        ...
        try:
            ...
            subject = await self._create_subject_from_chart_data(
                natal_chart_data
            )
            ...
            logger.info(
                f"ASYNC_KERYKEION_ARABIC_PARTS_SUCCESS: {elapsed:.3f}s"
            )
            return result
            ...
            return {
                "error": f"Async Arabic parts calculation failed: {str(e)}"
            }
    ...
    async def generate_chart_svg(
        ...
        try:
            ...
            logger.info(
                f"ASYNC_KERYKEION_SVG_SUCCESS: {name} in {elapsed:.3f}s"
            )
            return result
    ...
    async def batch_calculate_charts(
        ...
        logger.info(
            f"ASYNC_KERYKEION_BATCH_START: {len(chart_requests)} charts"
        )
        ...
        for request in chart_requests:
            task = self.get_full_natal_chart_data(
                ...
                house_system=HouseSystem(
                    request.get("house_system", "Placidus")
                ),
        ...
        for i, result in enumerate(results):
            if isinstance(result, Exception):
                logger.error(
                    f"ASYNC_KERYKEION_BATCH_ERROR: Chart {i} failed: {result}"
                )
        ...
        elapsed = time.time() - start_time
        logger.info(
            f"ASYNC_KERYKEION_BATCH_SUCCESS: {len(chart_requests)} charts in {elapsed:.3f}s"
        )

        return processed_results

    async def _create_subject_from_chart_data(
        ...
        try:
            ...
            birth_datetime = datetime.fromisoformat(
                subject_info.get("birth_datetime", "2000-01-01T00:00:00")
            )
    ...
    async def calculate_natal_chart_async(
        ...
        if elapsed_time > 2.0:  # Slow calculation threshold
            self.performance_stats["slow_calculations"] += 1
    ...
    def reset_performance_stats(self):
        ...
        logger.info("ASYNC_KERYKEION_STATS: Performance statistics reset")
    ...
    async def get_performance_stats(self) -> Dict[str, Any]:
        ...
        cache_efficiency = (
            (cached_ops / total_ops * 100) if total_ops > 0 else 0
        )
        ...
        return {
            "async_kerykeion_stats": {
                ...
                "average_calculation_time_ms": round(
                    self.performance_stats["average_calculation_time"] * 1000,
                    2,
                ),
                "slow_calculations": self.performance_stats[
                    "slow_calculations"
                ],
    ...
    async def precompute_popular_charts(self) -> Dict[str, Any]:
        ...
        logger.info(
            "ASYNC_KERYKEION_PRECOMPUTE_START: Popular charts pre-computation"
        )
        ...
        try:
            ...
            results = await self.batch_calculate_charts(
                popular_requests, use_cache=True
            )
            ...
            logger.info(
                f"ASYNC_KERYKEION_PRECOMPUTE_SUCCESS: {success_count}/{len(popular_requests)} charts in {elapsed_time:.3f}s"
            )
            ...
            return {
                ...
                "success_rate_percentage": (
                    success_count / len(popular_requests) * 100
                )
                if popular_requests
                else 0,
            }
