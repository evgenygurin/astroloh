...
class FeatureFlagService:
    ...
    def __init__(self):
        ...
        self.user_assignments: Dict[str, Set[str]] = {}  # user_id -> set of enabled features
        self.rollout_phases = {
        ...
        logger.info("FEATURE_FLAG_SERVICE_INIT: Feature flag service initialized")
    ...
    def _initialize_kerykeion_flags(self):
        ...
        logger.info(f"FEATURE_FLAG_SERVICE_INIT_FLAGS: Initialized {len(default_flags)} Kerykeion feature flags")
    
    def is_feature_enabled(self, feature_name: str, user_id: str) -> bool:
        """Check if a feature is enabled for a specific user."""
        try:
            op_id = performance_monitor.start_operation("FeatureFlagService", "is_feature_enabled")
            
            if feature_name not in self.flags:
                logger.warning(f"FEATURE_FLAG_UNKNOWN: {feature_name}")
                performance_monitor.end_operation(op_id, success=False, error_message="Unknown feature")
                return False
            ...
            logger.error(f"FEATURE_FLAG_ERROR: {feature_name} for {user_id}: {e}")
    ...
    async def get_user_enabled_features(self, user_id: str) -> Dict[str, bool]:
        ...
        for feature_name in self.flags:
            enabled_features[feature_name] = self.is_feature_enabled(feature_name, user_id)
        ...
        logger.debug(f"FEATURE_FLAG_USER_FEATURES: {user_id} has {len(self.user_assignments[user_id])} features enabled")
        ...
        try:
            ...
            if rollout_phase is not None:
                ...
                if rollout_phase in self.rollout_phases:
                    flag.rollout_percentage = self.rollout_phases[rollout_phase]
            ...
            logger.info(f"FEATURE_FLAG_UPDATED: {feature_name} -> phase={flag.rollout_phase.value}, percentage={flag.rollout_percentage}%")
    ...
    def advance_rollout_phase(self, feature_name: str) -> bool:
        ...
        if next_phase:
            return self.update_feature_flag(feature_name, rollout_phase=next_phase)
    ...
    def emergency_rollback_all(self) -> List[str]:
        ...
        logger.warning("FEATURE_FLAG_EMERGENCY_ROLLBACK_ALL: Rolling back all features")
    ...
    async def get_rollout_metrics(self, feature_name: str) -> Dict[str, Any]:
        ...
        total_users = max(1000, len(self.user_assignments))  # Estimate minimum users
        estimated_enabled_users = int(total_users * (flag.rollout_percentage / 100.0))
        
        # Get actual enabled users from assignments
        actual_enabled_users = sum(
            1 for user_features in self.user_assignments.values()
    ...
    async def update_feature_metrics(self, feature_name: str, metrics: Dict[str, Any]):
    ...
    async def save_state_to_cache(self):
        ...
        try:
            ...
            await astro_cache.set("feature_flags_state", state_data, ttl=86400)  # 24 hours
            logger.info("FEATURE_FLAG_STATE_SAVED: Feature flag state saved to cache")
            return True
    ...
    async def load_state_from_cache(self):
        ...
        try:
            ...
            if not state_data:
                logger.info("FEATURE_FLAG_NO_CACHED_STATE: No cached state found")
                return False
            ...
            for name, flag_data in state_data.get("flags", {}).items():
                try:
                    flag = FeatureFlag(
                        ...
                        rollout_phase=FeatureRolloutPhase(flag_data["rollout_phase"]),
                        rollout_percentage=flag_data["rollout_percentage"],
                        target_users=set(flag_data.get("target_users", [])),
                        excluded_users=set(flag_data.get("excluded_users", [])),
            ...
            for user_id, features in state_data.get("user_assignments", {}).items():
                self.user_assignments[user_id] = set(features)
            
            logger.info(f"FEATURE_FLAG_STATE_LOADED: Loaded {len(self.flags)} flags and {len(self.user_assignments)} user assignments")
            return True
