...
class CompatibilityAnalyzer:
    ...
    def __init__(self, synastry_service: SynastryService):
        ...
        logger.info(
            "COMPATIBILITY_ANALYZER_INIT: Compatibility analyzer initialized"
        )
    ...
    async def analyze_full_compatibility(
        ...
        try:
            ...
            # Композитная карта
            composite_chart = (
                await self.synastry_service.calculate_composite_chart(
                    person1, person2
                )
            )
            ...
            compatibility_report = {
                ...
                + zodiac_compatibility.get("strengths", []),
                "challenges": synastry.challenges
                + zodiac_compatibility.get("challenges", []),
                "advice": synastry.advice
                + zodiac_compatibility.get("advice", []),
                "relationship_themes": self._identify_relationship_themes(
                    synastry, zodiac_compatibility
                ),
            ...
            logger.error(
                f"COMPATIBILITY_FULL_ANALYSIS_ERROR: Analysis failed: {e}"
            )
            return self._create_fallback_report(
                person1, person2, compatibility_type
            )
    ...
    def _analyze_zodiac_compatibility(
        ...
        quality_compatibility = self._get_quality_compatibility(
            quality1, quality2
        )
        ...
        self._add_type_specific_advice(
            analysis, compatibility_type, final_score
        )

        return analysis

    def _get_element_compatibility(
        ...
        return (
            compatibility_matrix.get(key)
            or compatibility_matrix.get(reverse_key)
            or 50
        )

    def _get_quality_compatibility(
    ...
    def _add_element_interpretations(
        ...
        descriptions = element_descriptions.get(
            key
        ) or element_descriptions.get(reverse_key)
    ...
    def _add_quality_interpretations(
        ...
        quality_descriptions = {
            ("cardinal", "fixed"): {
                "strengths": [
                    "Инициатива + стабильность",
                    "Баланс действия и терпения",
                ],
                "challenges": ["Конфликт темпов", "Упрямство против напора"],
            },
            ("fixed", "mutable"): {
                "strengths": [
                    "Стабильность + адаптивность",
                    "Надежность + гибкость",
                ],
        ...
        descriptions = quality_descriptions.get(
            key
        ) or quality_descriptions.get(reverse_key)
    ...
    def _calculate_overall_score(
        ...
        overall_score = (
            synastry_score * synastry_weight + zodiac_score * zodiac_weight
        )

        return round(overall_score, 1)

    def _format_synastry_analysis(
        self, synastry: SynastryResult
    ) -> Dict[str, Any]:
        ...
        return {
            ...
            "major_aspects": [
                ...
                if aspect.get("aspect")
                in ["Соединение", "Оппозиция", "Трин", "Квадрат"]
            ][
                :5
            ],  # Топ-5 важных аспектов
    ...
    def _identify_relationship_themes(
        ...
        if len(synastry.aspects) > 10:
            ...
            themes.append(
                "Спокойные отношения с пространством для независимости"
            )
    ...
    def _assess_growth_potential(
        ...
        if quality_match < 60:
            growth_factors["development_areas"].append(
                "Синхронизация темпа жизни"
            )
    ...
    def _create_fallback_report(
        ...
        return {
            ...
            "zodiac_analysis": {
                "score": 50,
                "description": "Базовый анализ недоступен",
            },
