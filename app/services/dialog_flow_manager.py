...
class DialogFlowManager:
    ...
    def __init__(self):
        ...
        self.transition_conditions: Dict[Tuple[DialogState, YandexIntent], callable] = {
            (DialogState.COLLECTING_BIRTH_DATA, YandexIntent.HOROSCOPE): self._has_birth_date,
            (DialogState.COLLECTING_BIRTH_DATA, YandexIntent.NATAL_CHART): self._has_birth_date,
            (DialogState.COLLECTING_PARTNER_DATA, YandexIntent.COMPATIBILITY): self._has_partner_data,
        }
    ...
    def process_intent_in_flow(
        ...
        if next_state != current_state:
            ...
            self.logger.info(f"Dialog flow {flow.flow_id} transitioned from {current_state} to {next_state}")
    ...
    def _update_flow_context(self, flow: DialogFlow, entities: Dict[str, Any]) -> None:
        ...
        if entities.get("zodiac_signs"):
            ...
            elif "partner_zodiac" not in flow.context and len(entities["zodiac_signs"]) > 0:
    ...
    def _build_response_context(
        ...
        if next_state == DialogState.COLLECTING_BIRTH_DATA:
            ...
            context["missing_partner_info"] = self._get_missing_partner_info(flow.context)
    ...
    def _get_required_data(self, state: DialogState, context: Dict[str, Any]) -> List[str]:
        """Определяет, какие данные необходимы для текущего состояния."""
        required = []
        
        if state in [DialogState.COLLECTING_BIRTH_DATA, DialogState.PROVIDING_HOROSCOPE, DialogState.DISCUSSING_NATAL_CHART]:
    ...
    def _has_partner_data(self, flow: DialogFlow, entities: Dict[str, Any]) -> bool:
        ...
        return ("user_zodiac" in flow.context and "partner_zodiac" in flow.context) or \
               len(entities.get("zodiac_signs", [])) >= 2
    ...
    def get_flow_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "states": state_counts,
            "average_steps": total_steps / len(self.active_flows) if self.active_flows else 0
        }