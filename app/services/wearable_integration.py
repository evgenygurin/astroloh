...
class WearableIntegrationService:
    ...
    async def sync_wearable_data(
        ...
        try:
            # Get device
            devices = await self.iot_manager.get_user_devices(user_id, DeviceType.WEARABLE)
            ...
            insights = await self._generate_wearable_insights(user_id, data, lunar_correlation)
    ...
    async def _calculate_lunar_correlation(self, data: Dict[str, Any]) -> float:
        ...
        try:
            ...
            lunar_phase_value = lunar_info.get("phase_percentage", 0.5)  # 0-1 value
        ...
        try:
            ...
            if sleep_quality < 0.4:
                if current_phase in ["full_moon", "waxing_gibbous"]:
                    insights.append({
                        ...
                        "recommendations": ["meditation", "dim_lighting", "early_bedtime"],
                    })
            ...
            if stress_level > 0.7:
                insights.append({
                    ...
                    "recommendations": ["breathing_exercise", "calming_music", "aromatherapy"],
                })

            # Activity insights
            activity_level = data.get("activity_level", 0)
            if activity_level < 0.3 and current_phase in ["new_moon", "waning_crescent"]:
                insights.append({
                    "type": "activity",
                    "severity": "info",
                    "title": "Низкая активность в новолуние",
                    "message": "Новолуние - время для отдыха и восстановления. Это нормально!",
                    "recommendations": ["gentle_yoga", "journaling", "planning"],
                })
            ...
            if lunar_correlation > 0.7:
                insights.append({
                    ...
                    "recommendations": ["continue_tracking", "moon_rituals", "cycle_planning"],
                })
    ...
    async def send_wearable_notification(
        ...
        try:
            ...
            if not devices:
                return {"success": False, "message": "No wearable devices found"}
    ...
    async def schedule_transit_reminders(
        ...
        try:
            ...
            upcoming_transits = await self.transit_calculator.get_upcoming_transits(
                birth_data, days_ahead=7
            )

            if not upcoming_transits:
                return {"success": True, "message": "No upcoming transits", "reminders": []}

            # Filter significant transits
            significant_transits = [
                t for t in upcoming_transits 
                if t.get("significance", 0) >= 7  # Only high-significance transits
            ]
            ...
            logger.info(f"Scheduled {len(reminders_scheduled)} transit reminders for user {user_id}")
        ...
        try:
            ...
            query = select(WearableData).where(
                and_(
                    ...
                    WearableData.timestamp >= datetime.utcnow() - timedelta(days=recent_days),
                )
            ...
            if not recent_data:
                return {"success": False, "message": "No recent wearable data available"}

            # Analyze sleep patterns
            sleep_scores = [d.sleep_quality for d in recent_data if d.sleep_quality]
            avg_sleep_quality = sum(sleep_scores) / len(sleep_scores) if sleep_scores else 0
            ...
            phase_recommendations = {
                "new_moon": {
                    ...
                    "wake_time": "06:00", 
                    "tips": ["Perfect time for deep rest", "Set intentions before sleep", "Dark room recommended"],
                },
                "waxing_crescent": {
                    "optimal_bedtime": "22:00",
                    "wake_time": "06:30",
                    "tips": ["Growing energy", "Light stretching before bed", "Visualization exercises"],
                },
                "first_quarter": {
                    "optimal_bedtime": "22:30", 
                    "wake_time": "06:30",
                    "tips": ["Challenging phase for sleep", "Meditation crucial", "Avoid caffeine after 16:00"],
                },
                "waxing_gibbous": {
                    "optimal_bedtime": "22:00",
                    "wake_time": "06:00",
                    "tips": ["Refinement phase", "Review day before sleep", "Cool room temperature"],
                },
                "full_moon": {
                    "optimal_bedtime": "23:00",
                    "wake_time": "05:30",
                    "tips": ["Expect lighter sleep", "Extra meditation needed", "Eye mask recommended"],
                },
                "waning_gibbous": {
                    "optimal_bedtime": "21:30",
                    "wake_time": "06:00",
                    "tips": ["Gratitude practice", "Releasing meditation", "Warm bath before bed"],
                },
                "last_quarter": {
                    "optimal_bedtime": "22:00", 
                    "wake_time": "06:30",
                    "tips": ["Letting go phase", "Journal worries", "Deep breathing exercises"],
                },
                "waning_crescent": {
                    "optimal_bedtime": "21:00",
                    "wake_time": "06:30",
                    "tips": ["Maximum rest phase", "Longest sleep duration", "Complete darkness"],
                },
            }

            phase_rec = phase_recommendations.get(current_phase, phase_recommendations["new_moon"])
            ...
            if avg_sleep_quality < 0.6:
                recommendations.append({
                    ...
                    "actions": ["consistent_schedule", "sleep_hygiene", "limit_screens"],
                })

            # Lunar correlation recommendations
            lunar_correlations = [d.lunar_correlation for d in recent_data if d.lunar_correlation]
            if lunar_correlations:
                avg_correlation = sum(lunar_correlations) / len(lunar_correlations)
                if avg_correlation > 0.5:
                    recommendations.append({
                        "type": "lunar_sync",
                        "title": "Синхронизация с луной",
                        "message": "Ваш сон хорошо коррелирует с лунными фазами",
                        "actions": ["track_phases", "plan_activities", "honor_cycles"],
                    })
    ...
    async def create_wellness_plan(
        ...
        try:
            ...
            query = select(WearableData).where(
                and_(
                    WearableData.user_id == user_id,
                    WearableData.timestamp >= datetime.utcnow() - timedelta(days=7),
                )
            ).order_by(WearableData.timestamp.desc())
            ...
            for cycle in lunar_cycles:
                phase_plan = await self._create_phase_plan(cycle, goals, baseline_data)
            ...
            logger.info(f"Created {duration_days}-day wellness plan for user {user_id}")
    ...
    async def _calculate_lunar_cycles(self, duration_days: int) -> List[Dict[str, Any]]:
    ...
    async def _create_phase_plan(
        self, cycle: Dict[str, Any], goals: List[str], baseline_data: List[WearableData]
    ) -> Dict[str, Any]:
        ...
        return {
            ...
            "sleep_schedule": self._get_sleep_schedule_for_phase(cycle["focus"]),
            "activity_targets": self._get_activity_targets(cycle["focus"], baseline_data),
    ...
    def _get_sleep_schedule_for_phase(self, focus: str) -> Dict[str, str]:
        ...
        schedules = {
            "Новые начинания и планирование": {"bedtime": "22:00", "wake": "06:00"},
    ...
    def _get_activity_targets(self, focus: str, baseline: List[WearableData]) -> Dict[str, float]:
        """Get activity targets based on phase and baseline data."""
        if not baseline:
            return {"activity_level": 0.6, "stress_level": 0.4}

        avg_activity = sum(d.activity_level for d in baseline if d.activity_level) / len(baseline)
