...
class AIHoroscopeService:
    ...
    async def generate_natal_chart_interpretation(
        ...
        """
        logger.info(f"AI_NATAL_INTERPRETATION_START: {name}, focus={focus_area}")
        ...
        if self.astro_ai_service and settings.ENABLE_AI_GENERATION:
            try:
                ...
                result = await self.astro_ai_service.generate_natal_interpretation(
                    name=name,
                    birth_date=birth_date,
                    birth_time=birth_time,
                ...
                logger.info("AI_NATAL_INTERPRETATION_SUCCESS: Enhanced interpretation generated")
                return result
        ...
        return await self._generate_basic_natal_fallback(name, birth_date, focus_area)
    ...
    async def generate_enhanced_compatibility_analysis(
        ...
        if self.astro_ai_service and settings.ENABLE_AI_GENERATION:
            try:
                ...
                logger.info("AI_ENHANCED_COMPATIBILITY_SUCCESS: Advanced analysis generated")
                return result
    ...
    async def generate_transit_forecast_analysis(
        ...
        """
        logger.info(f"AI_TRANSIT_FORECAST_START: {zodiac_sign.value}, period={forecast_period}")
            ...
            try:
                ...
                if birth_date and birth_time and birth_place:
                    ...
                    if not natal_data.get("error"):
                        ...
                        logger.info("AI_TRANSIT_FORECAST_SUCCESS: Enhanced forecast generated")
                        return result
                
                # Fallback to basic sign-based forecast
                from app.services.astro_ai_service import ConsultationType
                result = await self.astro_ai_service.generate_personalized_advice(
                    zodiac_sign=zodiac_sign,
                    consultation_type=ConsultationType.TRANSIT_ANALYSIS,
                    user_context={"focus_area": focus_area, "period": forecast_period}
                )
                
                logger.info("AI_TRANSIT_FORECAST_SUCCESS: Basic forecast generated")
                return result
        ...
        return await self.generate_enhanced_horoscope(
            ...
            birth_date=birth_date,
            birth_time=birth_time,
            period=HoroscopePeriod.MONTHLY if forecast_period > 7 else HoroscopePeriod.DAILY,
            use_ai=True
        )
    ...
    async def generate_specialized_consultation(
        ...
        """
        logger.info(f"AI_SPECIALIZED_CONSULTATION_START: {zodiac_sign.value}, type={consultation_type}")
        ...
        if self.astro_ai_service and settings.ENABLE_AI_GENERATION:
            try:
                ...
                consultation_enum = consultation_map.get(consultation_type.lower(), ConsultationType.CAREER_GUIDANCE)
                
                # Get natal data if available
                natal_data = None
                if birth_data and birth_data.get("birth_date") and birth_data.get("birth_time") and birth_data.get("birth_place"):
                    try:
                        natal_data = self.astro_ai_service.kerykeion_service.get_full_natal_chart_data(
                            ...
                            timezone=birth_data.get("timezone", "Europe/Moscow")
                        )
    ...
    async def get_enhanced_service_status(self) -> Dict[str, Any]:
        ...
        if self.astro_ai_service:
            try:
                enhanced_status = await self.astro_ai_service.get_service_status()
    ...
    async def _generate_basic_natal_fallback(
        ...
        return {
            ...
            "zodiac_sign": zodiac_sign.value,
            "interpretation": advice.get("advice", "Характеристика недоступна"),
