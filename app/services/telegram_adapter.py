    ...
    def to_universal_request(self, telegram_data: Dict[str, Any]) -> UniversalRequest:
        ...
        try:
            ...
            if update.message:
                ...
                chat_id = str(callback.message.chat.id if callback.message else callback.user.id)
                message_id = str(callback.message.message_id if callback.message else callback.id)
    ...
    def from_universal_response(self, universal_response: UniversalResponse) -> Dict[str, Any]:
        ...
        try:
            ...
            if universal_response.platform_specific:
                ...
                callback_query_id = universal_response.platform_specific.get("callback_query_id")
            ...
            if universal_response.buttons:
                ...
                for button in universal_response.buttons:
                    keyboard_button = TelegramInlineKeyboard(
                        text=button.title,
                        callback_data=str(button.payload) if button.payload else button.title,
                        url=button.url,
                    )
                    keyboard_rows.append([keyboard_button])
                
                reply_markup = TelegramInlineKeyboardMarkup(inline_keyboard=keyboard_rows)
