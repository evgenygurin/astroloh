...
class GDPRComplianceService:
    ...
    async def get_user_data_summary(self, user_id: uuid.UUID) -> Optional[Dict[str, Any]]:
        """
        ...
        try:
            ...
            horoscope_count_result = await self.db.execute(
                select(func.count(HoroscopeRequest.id))
            ...
            summary = {
                ...
                "personal_data": {
                    ...
                    "has_birth_location": bool(birth_data and birth_data.get('birth_location')),
    ...
    async def export_user_data(self, user_id: uuid.UUID, format_type: str = "json") -> Optional[Dict[str, Any]]:
        """
    ...
    async def request_data_deletion(self, user_id: uuid.UUID, reason: str = None) -> str:
        ...
        """
        verification_code = await self.user_manager.request_data_deletion(user_id, reason)
    ...
    async def confirm_data_deletion(self, user_id: uuid.UUID, verification_code: str) -> bool:
        ...
        """
        success = await self.user_manager.confirm_data_deletion(user_id, verification_code)
    ...
    async def update_consent(self, user_id: uuid.UUID, consent: bool, 
                           retention_days: int = 365, 
                           consent_details: Dict[str, bool] = None) -> bool:
        """
        ...
        """
        success = await self.user_manager.set_data_consent(user_id, consent, retention_days)
    ...
    async def rectify_user_data(self, user_id: uuid.UUID, 
                              correction_data: Dict[str, Any]) -> bool:
        """
        ...
        try:
            ...
            if 'birth_date' in correction_data or 'birth_time' in correction_data or 'birth_location' in correction_data:
            ...
            if 'gender' in correction_data:
                updates['gender'] = SecurityUtils.sanitize_input(correction_data['gender'], 10)
    ...
    async def restrict_processing(self, user_id: uuid.UUID, restrict: bool, 
    ...
    async def generate_compliance_report(self, start_date: datetime = None, 
        ...
        consented_users_result = await self.db.execute(
            select(func.count(User.id)).where(User.data_consent == True)
        )
        ...
        deletion_requests_result = await self.db.execute(
            select(func.count(DataDeletionRequest.id))
            .where(DataDeletionRequest.requested_at.between(start_date, end_date))
        )
        ...
        security_events = {row[0]: row[1] for row in security_events_result.fetchall()}
...
class DataMinimizationService:
    ...
    def anonymize_analytics_data(data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if 'birth_date' in anonymized:
            ...
            anonymized['birth_season'] = DataMinimizationService._get_season(birth_date.month)
