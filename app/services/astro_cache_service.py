...
class AstroCacheService(CacheService):
    ...
    def __init__(self, redis_url: Optional[str] = None):
        ...
        logger.info("ASTRO_CACHE_SERVICE_INIT: Enhanced astrological caching initialized")
    ...
    async def get_natal_chart(
        ...
        birth_dt_str = birth_datetime.isoformat() if isinstance(birth_datetime, datetime) else birth_datetime
    ...
    async def set_natal_chart(
        ...
        success = await self.set(cache_key, chart_data, self.astro_ttl["natal_chart"])
        
        if success:
            logger.info(f"ASTRO_CACHE_SET: Natal chart cached {cache_key}")
        else:
            logger.error(f"ASTRO_CACHE_SET_ERROR: Failed to cache natal chart {cache_key}")
            
        return success

    async def get_daily_ephemeris(self, date: Union[date, str]) -> Optional[Dict[str, Any]]:
    ...
    async def set_daily_ephemeris(self, date: Union[date, str], ephemeris_data: Dict[str, Any]) -> bool:
        """Cache daily ephemeris data."""
        date_str = date.isoformat() if isinstance(date, date) else date
        cache_key = f"ephemeris:daily:{date_str}"
        
        return await self.set(cache_key, ephemeris_data, self.astro_ttl["daily_ephemeris"])
    ...
    async def get_current_transits(
        ...
        transit_dt_str = transit_date.isoformat() if isinstance(transit_date, datetime) else transit_date
        cache_key = self._generate_cache_key(
            "transits_current",
    ...
    async def set_current_transits(
        ...
        return await self.set(cache_key, transit_data, self.astro_ttl["current_transits"])
    ...
    async def get_period_forecast(
        ...
        start_dt_str = start_date.isoformat() if isinstance(start_date, date) else start_date
        cache_key = self._generate_cache_key(
            "forecast_period",
            chart_id=natal_chart_id,
            start=start_dt_str,
    ...
    async def set_period_forecast(
        ...
        return await self.set(cache_key, forecast_data, self.astro_ttl["period_forecasts"])
    ...
    async def set_compatibility_analysis(
        ...
        return await self.set(cache_key, compatibility_data, self.astro_ttl["compatibility"])
    ...
    async def set_arabic_parts(
        ...
        return await self.set(cache_key, parts_data, self.astro_ttl["arabic_parts"])
    ...
    async def set_chart_analysis(
        ...
        return await self.set(cache_key, analysis_data, self.astro_ttl["chart_analysis"])

    async def precompute_popular_data(self) -> Dict[str, Any]:
        """Pre-compute and cache popular astrological data."""
        logger.info("ASTRO_CACHE_PRECOMPUTE_START: Starting popular data pre-computation")
        ...
        try:
            ...
            await self.set(ephemeris_key, popular_ephemeris, self.astro_ttl["popular_calculations"])
            precomputed_data["ephemeris"] = True
            
            # Pre-compute popular zodiac sign combinations
            popular_combinations = [
                ("Leo", "Aries"), ("Scorpio", "Cancer"), ("Gemini", "Libra"),
                ("Virgo", "Capricorn"), ("Sagittarius", "Leo"), ("Aquarius", "Gemini")
            ]
            ...
            for sign1, sign2 in popular_combinations:
                ...
                await self.set(compat_key, popular_compatibility, self.astro_ttl["popular_calculations"])
            
            precomputed_data["compatibility_combinations"] = len(popular_combinations)
            ...
            await self.set(lunar_key, lunar_phases, self.astro_ttl["popular_calculations"])
            precomputed_data["lunar_phases"] = True
            
            elapsed_time = time.time() - start_time
            logger.info(f"ASTRO_CACHE_PRECOMPUTE_SUCCESS: Completed in {elapsed_time:.2f}s")
    ...
    def _update_performance_metrics(self, start_time: float, cache_hit: bool):
        ...
        new_avg = ((current_avg * (total_requests - 1)) + elapsed_time) / total_requests
    ...
    async def invalidate_user_data(self, user_identifier: str) -> int:
        ...
        logger.info(f"ASTRO_CACHE_INVALIDATE_COMPLETE: {invalidated_count} keys invalidated")
    ...
    async def clear_expired_cache(self) -> int:
        ...
        logger.info(f"ASTRO_CACHE_CLEANUP_COMPLETE: {cleared_count} expired entries cleared")
