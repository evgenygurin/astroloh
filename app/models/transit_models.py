...
class TransitData(BaseModel):
    ...
    active_transits: List[TransitAspect] = Field(
        default_factory=list, description="Активные транзиты"
    )
    approaching_transits: List[TransitAspect] = Field(
        default_factory=list, description="Приближающиеся транзиты"
    )
    summary: str = Field(..., description="Краткое резюме транзитов")
    daily_influences: List[str] = Field(
        default_factory=list, description="Ключевые влияния дня"
    )
...
class ProgressionData(BaseModel):
    ...
    progressed_houses: Dict[int, Dict[str, Any]] = Field(
        default_factory=dict, description="Прогрессированные дома"
    )
    interpretation: ProgressionInterpretation = Field(
        ..., description="Интерпретация прогрессий"
    )
    key_changes: List[str] = Field(
        default_factory=list, description="Ключевые изменения"
    )
...
class SolarReturnData(BaseModel):
    ...
    planets: Dict[str, Dict[str, Any]] = Field(
        default_factory=dict, description="Позиции планет в соляре"
    )
    houses: Dict[int, Dict[str, Any]] = Field(
        default_factory=dict, description="Дома соляра"
    )
    interpretation: SolarReturnInterpretation = Field(
        ..., description="Интерпретация соляра"
    )
...
class LunarReturnData(BaseModel):
    ...
    planets: Dict[str, Dict[str, Any]] = Field(
        default_factory=dict, description="Позиции планет в лунаре"
    )
    houses: Dict[int, Dict[str, Any]] = Field(
        default_factory=dict, description="Дома лунара"
    )
...
class YearlyForecast(BaseModel):
    ...
    monthly_highlights: Dict[int, str] = Field(
        default_factory=dict, description="Ключевые моменты по месяцам"
    )
    seasonal_themes: Dict[str, str] = Field(
        default_factory=dict, description="Сезонные темы"
    )
    success_indicators: List[str] = Field(
        default_factory=list, description="Индикаторы успеха"
    )
    caution_periods: List[str] = Field(
        default_factory=list, description="Периоды осторожности"
    )
...
class MonthlyGuidance(BaseModel):
    ...
    weekly_rhythms: Dict[int, str] = Field(
        default_factory=dict, description="Недельные ритмы"
    )
    emotional_cycles: List[str] = Field(
        default_factory=list, description="Эмоциональные циклы"
    )
    optimal_actions: List[str] = Field(
        default_factory=list, description="Оптимальные действия"
    )
