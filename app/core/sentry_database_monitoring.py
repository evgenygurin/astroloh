    ...
    def setup_engine_monitoring(self, engine: Engine):
        ...
        if self.connection_pool_warnings:
            ...
            def on_connect(dbapi_connection, connection_record):
                with sentry_sdk.configure_scope() as scope:
                    scope.set_context("db_connection", {
                        "event": "connect",
...
def trace_database_operation(
    ...
    **operation_metadata
    ...
    with start_span(
        op="db.sql.query",
        description=f"Database {operation_type.upper()}" + (f" on {table_name}" if table_name else "")
    ) as span:
        # Теги базы данных
...
async def trace_asyncpg_operation(
    ...
    **operation_metadata
    ...
    with start_span(
        op="db.sql.asyncpg",
        description=f"AsyncPG {operation_type.upper()}"
    ) as span:
        # Теги AsyncPG
        span.set_tag("db.system", "postgresql")
        ...
        if query:
            ...
            span.set_data("db.statement", safe_query[:100] + "..." if len(safe_query) > 100 else safe_query)
...
async def trace_astro_cache_operation(
    ...
    **cache_metadata
    ...
    with start_span(
        ...
        span.set_tag("cache.key_prefix", cache_key.split(':')[0] if ':' in cache_key else cache_key)
    ...
    **batch_metadata
    ...
    with start_span(
        ...
        try:
            ...
            span.set_data("db.batch.throughput_per_sec", batch_size / (duration_ms / 1000))
...
async def monitor_connection_pool_health(pool_name: str = "main"):
    ...
    try:
        ...
        if pool_stats["active_connections"] >= pool_stats["max_connections"] * 0.9:
            sentry_sdk.capture_message(
                f"Connection pool {pool_name} почти исчерпан",
                level="warning"
            )
