...
class GoogleAssistantSettings(BaseSettings):
    ...
    DIALOGFLOW_SESSION_TIMEOUT: int = Field(
        default=300, env="DIALOGFLOW_SESSION_TIMEOUT"
    )

    # Google Actions settings
    ACTIONS_CONSOLE_PROJECT_ID: Optional[str] = Field(
        default=None, env="GOOGLE_ACTIONS_PROJECT_ID"
    )
...
class MultiPlatformSettings(BaseSettings):
    ...
    ENABLE_CROSS_PLATFORM_ANALYTICS: bool = Field(
        default=True, env="ENABLE_CROSS_PLATFORM_ANALYTICS"
    )
    ...
    USER_SYNC_STRATEGY: str = Field(
        default="opt_in", env="USER_SYNC_STRATEGY"
    )  # opt_in, opt_out, automatic

    # Content adaptation
    ADAPTIVE_RESPONSES: bool = Field(default=True, env="ADAPTIVE_RESPONSES")
    PLATFORM_SPECIFIC_CONTENT: bool = Field(
        default=True, env="PLATFORM_SPECIFIC_CONTENT"
    )

    # Rate limiting and security
    GLOBAL_RATE_LIMIT: int = Field(
        default=100, env="GLOBAL_RATE_LIMIT"
    )  # requests per minute
    ENABLE_WEBHOOK_VERIFICATION: bool = Field(
        default=True, env="ENABLE_WEBHOOK_VERIFICATION"
    )
