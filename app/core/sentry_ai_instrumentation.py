...
def trace_ai_agent(
    ...
    **agent_metadata
    ...
    with start_span(
        ...
        if input_data:
            span.set_data("ai.agent.input", json.dumps(input_data, ensure_ascii=False))
...
def trace_ai_client(
    ...
    **client_params
    ...
    """
    with start_span(
        op=f"gen_ai.{operation}",
        description=f"AI Client: {model_name}"
    ) as span:
        ...
        if messages:
            ...
            span.set_data("ai.messages", json.dumps(safe_messages, ensure_ascii=False))
    ...
    with start_span(
        ...
        if tool_input:
            ...
            for key, value in tool_input.items():
                if key not in ["birth_time", "birth_place", "user_id", "session_id"]:
                    safe_input[key] = value
                else:
                    safe_input[key] = "[FILTERED]"
            
            span.set_data("ai.tool.input", json.dumps(safe_input, ensure_ascii=False))
...
def trace_natal_chart_ai_interpretation(
    ...
    with trace_ai_agent(
        ...
        if "zodiac_sign" in safe_birth_data:
            span.set_tag("astro.natal.sun_sign", safe_birth_data["zodiac_sign"])
