...
async def test_ai_instrumentation(request: SentryTestRequest):
    ...
    with trace_horoscope_ai_generation(
        ...
        with trace_ai_client(
            ...
            messages=[
                {
                    "role": "system",
                    "content": "Ты - профессиональный астролог",
                },
                {
                    "role": "user",
                    "content": f"Дай гороскоп для знака {request.zodiac_sign or 'leo'}",
                },
            ],
            temperature=0.7,
            max_tokens=400,
        ...
        results[
            "ai_horoscope"
        ] = f"✅ AI гороскоп сгенерирован ({len(generated_text)} символов)"
    ...
    with trace_ai_tool_execution(
        ...
        results[
            "ai_tool"
        ] = f"✅ Планетарные расчеты выполнены: {len(planetary_data['aspects'])} аспектов"
...
async def test_database_monitoring(request: SentryTestRequest):
    ...
    with trace_astro_database_operation(
        ...
        results[
            "astro_database"
        ] = f"✅ Гороскоп сохранен для {request.zodiac_sign or 'leo'}"

    # ==================== CACHE OPERATIONS ====================
    async with trace_astro_cache_operation(
        cache_operation="get",
        cache_key="ephemeris:2025-08-10",
    ...
    with trace_database_operation(
        ...
        results[
            "regular_database"
        ] = "✅ Пользовательские данные загружены (150 записей)"
...
async def test_async_monitoring(request: SentryTestRequest):
    ...
    batch_signs = (
        ["leo", "virgo", "libra"]
        if not request.zodiac_sign
        else [request.zodiac_sign]
    )
    ...
    async with trace_async_context(
        ...
        results[
            "async_context"
        ] = "✅ Комплексный астрологический анализ завершен"
...
async def test_all_enhanced_sentry_features():
    ...
    with trace_horoscope_ai_generation(
        zodiac_sign="sagittarius", period="weekly", ai_model="yandex-gpt-lite"
    ):
        # AI генерация
        with trace_ai_client(
            operation="chat",
            model_name="yandex-gpt-lite",
            messages=[
                {"role": "user", "content": "Weekly horoscope for Sagittarius"}
            ],
            temperature=0.8,
        ) as ai_span:
            ...
            track_ai_response_quality(
                ai_span, response_length=645, confidence_score=0.89
            )
