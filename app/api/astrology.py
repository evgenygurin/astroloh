...
async def get_natal_chart(
    birth_data: BirthData, current_user: User = Depends(get_current_user)
):
    ...
    try:
        ...
        interpretation = chart_data.get(
            "interpretation", "Натальная карта рассчитана успешно."
        )
...
async def get_horoscope(sign: str, type: str):  # daily, weekly, monthly
    ...
    try:
        ...
        horoscope_data = horoscope_generator.generate_personalized_horoscope(
            zodiac_sign=zodiac_sign_enum, period=period_enum
        )
        ...
        return HoroscopeResponse(
            ...
            lucky_color=horoscope_data.get("lucky_colors", ["белый"])[0]
            if horoscope_data.get("lucky_colors")
            else "белый",
        )
...
async def get_compatibility(
    ...
    try:
        ...
        logger.info(
            "Compatibility calculation requested for authenticated user"
        )
