...
async def google_assistant_webhook(
    ...
    try:
        ...
        if not google_adapter.validate_request(google_data):
            raise HTTPException(status_code=400, detail="Invalid Google Assistant request")
        ...
        if universal_request.user_context and "conversation_token" in universal_request.user_context:
            platform_specific["conversation_token"] = universal_request.user_context["conversation_token"]
        ...
        try:
            ...
            if "queryResult" in google_data:
                ...
                from app.models.google_models import GoogleResponse, GoogleRichResponse, GoogleSimpleResponse
                ...
                rich_response = GoogleRichResponse(
                    items=[{"simpleResponse": simple_response.dict()}]
                )
...
async def google_assistant_health_check() -> Dict[str, Any]:
    ...
    try:
        ...
        logger.error(f"Google Assistant health check failed: {str(e)}")
