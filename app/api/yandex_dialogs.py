...
async def yandex_webhook(
            ...
            **log_context,
            "request_data": {
                ...
                "application_id": getattr(
                    request.session.application, "application_id", "unknown"
                )
                if hasattr(request.session, "application")
                else "unknown",
                "user_utterance": request.request.original_utterance[:200]
                if request.request.original_utterance
                else "",
                "command": getattr(request.request, "command", "")
                if hasattr(request.request, "command")
                else "",
                "type": getattr(request.request, "type", "SimpleUtterance")
                if hasattr(request.request, "type")
                else "SimpleUtterance",
                "has_markup": bool(getattr(request.request, "markup", None)),
                "has_payload": bool(getattr(request.request, "payload", None)),
                "has_interfaces": bool(
                    getattr(request.meta, "interfaces", None)
                )
                ...
                "client_id": getattr(request.meta, "client_id", "unknown")
                if hasattr(request, "meta")
                else "unknown",
            },
        },
    ...
    try:
        logger.info(
            "USER_MANAGER_INIT",
            extra={**log_context, "step": "initializing_user_manager"},
        )
        ...
        logger.info(
            ...
            extra={**log_context, "step": "dialog_processing"},
        )
        ...
        logger.info(
            ...
            extra={
                **log_context,
                ...
                "response_data": {
                    "text": dialog_response.response.text[:150]
                    if hasattr(dialog_response, "response")
                    and hasattr(dialog_response.response, "text")
                    else "",
                    "tts": dialog_response.response.tts[:100]
                    if hasattr(dialog_response, "response")
                    and hasattr(dialog_response.response, "tts")
                    and dialog_response.response.tts
                    else None,
                    "has_buttons": bool(
                    ...
                    "end_session": getattr(
                        dialog_response.response, "end_session", False
                    )
                    if hasattr(dialog_response, "response")
                    else False,
                },
        ...
        error_response = error_handler.handle_error(
            e,
            {
                "user_id": request.session.user_id,
                "session_id": request.session.session_id,
                "request_text": request.request.original_utterance[:100]
                if request.request.original_utterance
                else "",
                "correlation_id": correlation_id,
            },
