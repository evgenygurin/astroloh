...
async def get_lunar_calendar(
    year: int, month: int, db: AsyncSession = Depends(get_db_session)
):
...
async def get_current_phase(db: AsyncSession = Depends(get_db_session)):
    ...
    try:
        ...
        return CurrentPhaseResponse(
            ...
            best_activities=energy_data.get(
                "best_activities", ["meditation", "planning"]
            ),
        )
...
async def get_phase_recommendations(
    phase_name: str, db: AsyncSession = Depends(get_db_session)
):
...
async def get_lunar_day_info(
    day: int, db: AsyncSession = Depends(get_db_session)
):
