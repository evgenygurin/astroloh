...
async def get_feature_metrics(feature_name: str):
    ...
    try:
        logger.info(
            f"DEPLOYMENT_API_FEATURE_METRICS: Getting metrics for {feature_name}"
        )
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get feature metrics: {str(e)}"
        )


@router.put("/features/{feature_name}")
async def update_feature_flag(
    feature_name: str, update_data: FeatureFlagUpdate
):
    ...
    try:
        ...
        success = feature_flags.update_feature_flag(
            ...
            target_users=set(update_data.target_users)
            if update_data.target_users
            else None,
            excluded_users=set(update_data.excluded_users)
            if update_data.excluded_users
            else None,
        )
...
async def advance_feature_phase(feature_name: str):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to advance phase: {str(e)}"
        )
...
async def emergency_feature_rollback(feature_name: str):
    ...
    try:
        logger.warning(
            f"DEPLOYMENT_API_EMERGENCY_ROLLBACK: Emergency rollback for {feature_name}"
        )
        ...
        return {
            "success": True,
            "message": f"Feature {feature_name} emergency rollback completed",
        }
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to execute emergency rollback: {str(e)}",
        )


@router.post("/rollback/manual")
async def manual_rollback(
    rollback_request: ManualRollbackRequest, background_tasks: BackgroundTasks
):
    """Execute manual rollback of specified features."""
    try:
        logger.warning(
            f"DEPLOYMENT_API_MANUAL_ROLLBACK: Manual rollback requested for {len(rollback_request.features)} features"
        )
        ...
        for feature in rollback_request.features:
            if feature not in feature_flags.flags:
                raise HTTPException(
                    status_code=404, detail=f"Feature not found: {feature}"
                )
        ...
        def execute_rollback():
            ...
            try:
                ...
                logger.info(
                    f"DEPLOYMENT_API_MANUAL_ROLLBACK_COMPLETE: {rollback_event.rollback_id}"
                )
            except Exception as e:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to initiate manual rollback: {str(e)}",
        )
...
async def get_rollback_history(limit: int = Query(10, ge=1, le=100)):
    ...
    try:
        logger.info(
            f"DEPLOYMENT_API_ROLLBACK_HISTORY: Getting last {limit} rollbacks"
        )
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get rollback history: {str(e)}"
        )
...
async def get_rollback_statistics():
    ...
    try:
        logger.info(
            "DEPLOYMENT_API_ROLLBACK_STATS: Getting rollback statistics"
        )
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to get rollback statistics: {str(e)}",
        )
...
async def get_health_checks():
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get health checks: {str(e)}"
        )
...
async def get_performance_report(hours: int = Query(24, ge=1, le=168)):
    ...
    try:
        logger.info(
            f"DEPLOYMENT_API_PERFORMANCE_REPORT: Getting {hours}h report"
        )
...
async def get_system_status():
    ...
    try:
        logger.info(
            "DEPLOYMENT_API_SYSTEM_STATUS: Getting comprehensive system status"
        )
...
async def start_deployment_monitoring():
    ...
    try:
        logger.info(
            "DEPLOYMENT_API_START_MONITORING: Starting deployment monitoring"
        )
...
async def stop_deployment_monitoring():
    ...
    try:
        logger.info(
            "DEPLOYMENT_API_STOP_MONITORING: Stopping deployment monitoring"
        )
...
async def submit_user_feedback(
    ...
    try:
        logger.info(
            f"DEPLOYMENT_API_USER_FEEDBACK: User {user_id} rated {rating}/10"
        )
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to record user feedback: {str(e)}"
        )
...
async def get_kerykeion_feature_usage():
    ...
    try:
        logger.info(
            "DEPLOYMENT_API_KERYKEION_USAGE: Getting Kerykeion feature usage"
        )
        ...
        return {
            ...
            "summary": {
                ...
                "enabled_features": sum(
                    1 for f in usage_stats.values() if f.get("enabled", False)
                ),
                "total_estimated_users": sum(
                    f.get("estimated_enabled_users", 0)
                    for f in usage_stats.values()
                ),
            },
        }
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get Kerykeion usage: {str(e)}"
        )
...
async def check_deployment_alerts():
    ...
    try:
        logger.info(
            "DEPLOYMENT_API_ALERTS_CHECK: Checking for deployment alerts"
        )
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to check alerts: {str(e)}"
        )
