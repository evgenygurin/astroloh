...
def test_kerykeion_service():
    ...
    print(f"✅ KerykeionService created")
    ...
    print(f"📊 Chart shape: {chart_data.get('chart_shape', {}).get('shape', 'Unknown')}")
    ...
    for planet_name in ['sun', 'moon', 'mercury', 'venus', 'mars']:
        if planet_name in planets:
            ...
            print(f"  {planet.get('name', planet_name)}: {planet.get('sign', 'Unknown')} "
    ...
    if arabic_parts:
        ...
        for part_name, part_data in list(arabic_parts.items())[:3]:
            print(f"  {part_data.get('name', part_name)}: {part_data.get('sign', 'Unknown')} "
...
def test_enhanced_natal_chart():
    ...
    print(f"✅ Enhanced chart calculated")
    print(f"🔧 Backend used: {enhanced_chart.get('calculation_backend', 'unknown')}")
    print(f"🌟 Enhanced features available: {enhanced_chart.get('enhanced_features_available', False)}")
    ...
    if enhanced_chart.get('enhanced_features_available'):
        ...
        if aspects:
            ...
            for aspect in aspects:
                print(f"  {aspect.get('planet1', '?')} {aspect.get('symbol', '?')} "
...
def test_astrology_calculator_backend():
    ...
    print(f"✅ AstrologyCalculator created")
...
def main():
    ...
    if success_count == total_tests:
        print("🎉 All tests passed! Kerykeion integration is working correctly.")
        return True
    else:
