...
class DeploymentOrchestrator:
    ...
    async def execute_deployment(
        ...
        try:
            ...
            init_result = (
                await self.deployment_config.initialize_deployment_systems()
            )
            ...
            print(
                f"   - Health score: {init_result['initial_health_score']:.1f}%"
            )
            print(
                f"   - Systems ready: {len(init_result['systems_initialized'])}"
            )
            ...
            if not self.auto_advance and not self._confirm_action(
    ...
    def _show_deployment_plan(self, start_phase: str = None):
        ...
        for phase_name, config in phases.items():
            ...
            status = (
                "⏭️ " if start_phase and phase_name != start_phase else "📋"
            )
    ...
    async def _handle_interruption(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if health_score < 70:
                print(
                    f"⚠️  System health low ({health_score:.1f}%), initiating rollback..."
                )
                rollback_result = (
                ...
                print(
                    f"✅ System health acceptable ({health_score:.1f}%), no rollback needed"
                )
                return {
                    "success": False,
                    "interrupted": True,
    ...
    async def show_current_status(self):
        ...
        try:
            ...
            print(
                f"🖥️  System Status: {'✅ Running' if system_status.get('startup_completed') else '⚠️ Initializing'}"
            )
            ...
            enabled_count = sum(
                1 for f in features_status["features"].values() if f["enabled"]
            )
            print(
                f"✅ Enabled Features: {enabled_count}/{features_status['total_features']}"
            )
            ...
            print(
                f"🔄 Rollback History: {rollback_stats['total_rollbacks']} total"
            )
        ...
        try:
            if features:
                ...
                rollback_result = await rollback_system.manual_rollback(
                    features=features, reason="Manual rollback via script"
                )
            else:
            ...
            if rollback_result.get("success"):
                ...
                if "recovery_time" in rollback_result:
                    print(
                        f"   Recovery time: {rollback_result['recovery_time']:.1f}s"
                    )
            else:
                print(
                    f"❌ Rollback failed: {rollback_result.get('error_message', 'Unknown error')}"
                )
...
async def main():
    ...
    parser = argparse.ArgumentParser(
        description="Deploy Kerykeion features to production"
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
    ...
    parser.add_argument(
        "--phase",
        choices=["phase_1", "phase_2", "phase_3", "full"],
        help="Start from specific phase",
    )
    parser.add_argument(
        "--auto-advance",
        action="store_true",
        help="Automatically advance without confirmations",
    )
    parser.add_argument(
        "--rollback", action="store_true", help="Execute emergency rollback"
    )
    parser.add_argument(
    ...
    orchestrator = DeploymentOrchestrator(
        dry_run=args.dry_run, auto_advance=args.auto_advance
    )
        ...
        if args.status:
            ...
            if result.get("success") or result.get("dry_run"):
                ...
                if result.get("deployment"):
                    ...
                    print(
                        f"   Phases completed: {len(deployment.get('phases_completed', []))}"
                    )
                    if "total_duration_minutes" in deployment:
                        print(
                            f"   Total duration: {deployment['total_duration_minutes']:.1f} minutes"
                        )
            else:
                ...
                if result.get("phases_failed"):
                    print(
                        f"   Failed phases: {', '.join(result['phases_failed'])}"
                    )
            ...
            sys.exit(
                0 if result.get("success") or result.get("dry_run") else 1
            )
